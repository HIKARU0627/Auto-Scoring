...
/// 知らない。** 採点が終わっても答案の `state` は `ai_processed` のままだし、
/// そもそも採点が**始まったかどうか**すら `state` には出ない -- 起票の有無は
/// `listJobs` にしか現れず、ホームはそれを引かないと決めている (§4)。
/// Issue #80 で答案取込画面が取込直後に起票するようになったが、409や通信断で
/// 失敗した答案と、その仕組みより前に取り込まれた答案は、`ai_processed` の
/// ままジョブ0件で止まっている。文言はそこを跨がない
...
enum HomeWorkBucket {
  ...
  /// `ai_processed` -- 取込と回答欄の抽出が終わり、人がまだ確認していない
  /// もの。**AI採点がどこまで進んだかは、始まったかどうかも含めて分からない**:
  /// 起票済みで実行待ち・AI処理中・採点が終わってレビュー待ち に加えて、
  /// **起票そのものが行われていない**答案（起票が失敗した、または Issue #80
  /// より前に取り込まれた）も、すべて `ai_processed` である (§3.1)。
  ///
  /// ラベルを「採点・レビュー待ち」にしてあるのは、この bucket について
  /// 言えるのが「採点もレビューもまだ済んでいない」ことだけだからである。
  /// 「レビュー待ち」だけだと「AIは終わっている」の断定になり、
  /// 「採点中」だと「動いている」の断定になる。どちらもホームは知らない。
  awaitingReview(
    label: '採点・レビュー待ち',
