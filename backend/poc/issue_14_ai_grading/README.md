...
A cell with no recorded `response` yet (a real sample staged ahead of API
credentials) is counted as "pending", not scored as zero. A cell whose raw
JSON fails schema validation is a schema violation -- it is never
free-text-parsed into a guessed grade (Issue #14 acceptance). Calling a real
Gemini / Claude / GPT endpoint to populate `recorded` (the "live-provider
path") is not implemented in this PoC yet; it is added once credentials are
available, following the same deferral `poc/run_ocr_eval.py` (PoC 1) used.
