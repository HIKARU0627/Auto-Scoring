...
class TestConfirmGate:
    ...
    def test_a_question_split_across_pages_says_what_to_do_about_it(
        ...
        with SqlAlchemyUnitOfWork(_session_factory(data_root)) as uow:
            ...
            assert questions[0].page_2 == 2
            assert questions[0].answer_area is not None
            assert questions[0].answer_area_2 is not None

    def test_a_question_split_across_three_pages_says_what_to_do_about_it(
        self, client: TestClient, data_root: Path, detector: _FakeDetector
    ) -> None:
        """Issue #108: A question split across 3 or more pages is rejected with 422
        naming the 2-page limit.
        """
        detector._body = _detection_body(_area(page=1), _area(page=2), _area(page=3))
        test_id = _register_test(client)
        _confirm_questions(data_root, test_id, "問1")
        _upload_layout(client, test_id, pages=3)
        detected = _detect(client, test_id).json()
