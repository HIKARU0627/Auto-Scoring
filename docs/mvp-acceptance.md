...
`NullOCRProvider` が入る（判断 A の実アダプタが未実装のため、これが現状の既定）。
confidence は常に 0.0 で閾値を下回るため、**全設問が `usable=False`** になる。
同モジュール末尾の2本がその現実を固定している。

- 依存エッジが無ければ、採点結果は残り、人が全設問を確認すれば**出力まで到達する**。
  失われるのは自動確定であって、経路そのものではない。
- 依存エッジがあると、**後続設問は `blocked` のまま動かない**（`evaluate_readiness` は
  usable でない前提設問で後続を解放しない）。今日そこを通す手段は人による
  `resume` だけである。この扱いを変えるかどうかは **Issue #114** で決める。

「動くはず」ではなく「いまこうなる」を書いてある。#114 で変われば、この2本が落ちて
そう言う。
