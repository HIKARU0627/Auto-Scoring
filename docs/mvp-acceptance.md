...
| 箇所                                                       | 内容                                                                                           |
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `test_no_student_identifying_data_reaches_either_provider` | 決定 7 より前のルール（ページ全体の画像の送信禁止）を固定している（§7）                        |
| OCR を前提にした採点処理                                   | 決定 10 で OCR は採点の前提から外れた（簡易設計書 §8.1）。OCR 失敗で採点を止めない             |
| `recognition` フィールドの出どころ                         | 採点AIの読み取りと OCR の読み取りを**別物として保持**する必要がある（簡易設計書 §8.1.3・§9.2） |
