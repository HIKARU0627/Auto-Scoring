...
| 2   | 模範解答PDF読込（**Issue #95で必須入力から除外。§31の実装項目も差し替え済み**） | 同上（`POST /tests` の `model_answer`）。`MODEL_ANSWER` regionが `Question.model_answer` になり、無ければ採点自体が拒否されることまで通る                                                                                      | `test_test_intake.py`, `test_grading_processor.py`                                                                                             |
...
| [#54](https://github.com/HIKARU0627/Auto-Scoring/issues/54) | live OCR/AI provider での疎通・schema・cleanup | A・B は Issue #81 で確定済み。残るブロックは**実APIキー**と実アダプタ実装 |
...
> **⚠ 本節の検査は Issue #95 決定 7 より前のルールを固定している。**
> 決定 7 により、採点資料・生徒答案とも採用 provider へ送ってよくなり、
> **ページ全体の画像の送信も許可された**（`business-rules-and-evaluation-data.md` §2 (2)、
> 簡易設計書 §26.1）。下表の「ページ全体のラスタ画像＝禁止」「当該設問以外の
> `question_id`＝禁止」は**現行仕様と食い違う**。
> 実装（`test_no_student_identifying_data_reaches_either_provider`）は旧ルールのままであり、
> **決定 7 に合わせた見直しが要る**。見直しの際も
> 「provider へ送ってよい」と「公開してよい」は別であることを保ったまま行うこと
> （commit / PR / Issue / docs / CI ログ / スクリーンショット / fixture への持ち出しは引き続き禁止）。
> 本 Issue はドキュメントのみの変更のため、テストは変更していない。
