...
> **Issue #101 で入力仕様と画面が変わった。**
> テスト登録は「模範解答PDF + 採点マニュアルPDF」ではなく
> 「採点基準PDF（必須）+ 役割つきの任意資料」を受け取る。
> 「テスト登録画面」と「答案取込画面」は 資料取込画面 (`/intake`) に統合された。
> 答案の取込・前処理そのもの（本書の主題）は変えていない。
...
- `Test` に登録済みの全設問が参照するページ集合（`{p for q in questions for p in q.pages}`、
  Issue #108 で 2 ページにまたがる設問の `page_2` も含む）を「期待ページ集合」とする。
...
#### 実機再検証 #6 で閾値を 1.23 倍上回った切り出しが採点へ流れた（Issue #214）

GitHub Issue [#214](https://github.com/HIKARU0627/Auto-Scoring/issues/214)。
実機再検証 #6（API 経由・11 教科）で、人の目では筆跡が 1 文字も入っていない
切り出し（罫線のみ）が `NEARLY_BLANK_INK_COVERAGE = 0.002` を **1.23 倍**
上回り（`0.002455`）、採点へ送られて 0 点・確信度 1.00 になった。
本 Issue は**先に測る**ものであり、閾値をいきなり動かさない。

##### `ink_coverage` は面積で正規化されているか

**はい。** `adapters.image.ink.ink_coverage` はグレースケール画像の
「インクレベル以下の画素数 ÷ 全画素数」であり、**すでに切り出し面積で
正規化された比率**である（0.0〜1.0）。生のインク画素数ではない。
`backend/tests/test_page_ink.py::test_ink_coverage_is_the_ink_pixel_fraction` が
合成画像で固定する。

したがって Issue #214 で検討した案 B（面積で正規化する）は**不要**である。
ただし、**同じ印刷罫線だけが入った切り出しは、切り出しが小さいほど比率が
高く読まれる**（面積が分母なので、分子のインク画素数がほぼ一定なら coverage は
面積に反比例する）。合成確認: 同じ横罫線 1 本を 100×200 と 400×800 に描くと
coverage は約 0.03 と 0.0075（4 倍差）。実機 #6 の問題切り出し
（655×74 px、面積 48,470）を白紙で 2 倍にパディングすると coverage は
`0.002455 → 0.000614` まで下がる。**大きい切り出しなら現行閾値で止まるが、
細長く小さい切り出しだけが上振れして通り抜ける**、という非対称がある。

##### 実測（実機再検証 #6、回答欄が作られた切り出し）

製品と同じ `ink_coverage` で再計算し、Issue 本文の分布と一致した
（不一致 0 件）。測定スクリプト:
`backend/poc/issue_214_blank_crop_threshold/report.py --data <live-run dir>`。

| 分母                                                  |                         件数 |
| ----------------------------------------------------- | ---------------------------: |
| 回答欄が作られた設問（ink coverage を測れた切り出し） |                       **23** |
| 閾値 0.002 以下として止まったもの                     |                   **0 / 23** |
| 人の目で筆跡が 1 文字も入っていないと判定したもの     | **1 / 23**（ink `0.002455`） |

分布（23 件、小さい順。末尾 2 組は同一答案の重複）:

```text
0.002455  0.008840  0.014613  0.017140  0.024943  0.054018  0.068627  0.073770
0.076780  0.076885  0.086111  0.087947  0.089411  0.097268  0.105979  0.125673
0.129492  0.140221  0.145451  0.160988  0.160988  0.208319  0.208319
```

面積（px²）と ink coverage の関係（23 件）:

| 回帰                  | 係数           | 相関 r |
| --------------------- | -------------- | -----: |
| log(面積) vs log(ink) | 傾き −0.36     |  −0.42 |
| 面積 vs ink（線形）   | 傾き −1.2×10⁻⁷ |  −0.50 |

面積と coverage は弱い負の相関に留まる。**面積 10,080 px² で ink 0.077 の
切り出しと、面積 48,470 px² で ink 0.002 の切り出しが同じ 23 件に共存する**
ため、「小さい切り出し＝低 coverage」にはならない。面積だけでは閾値を
補正できない。

##### 閾値を上げる案（A）をこの 23 件だけで採用できるか

`0.002455` と次に小さい `0.008840` のあいだに閾値を置けば、**この 23 件では**
1 件だけ止めて 22 件を通す（3.6 倍のギャップ）。しかし **採用しなかった**。
理由は Issue #158（OCR usable 判定）と同じ用心である:

1. **分母が 23 件・実機再検証 1 回だけ**である。#158 は「記録済み 15 件からは
   どんな閾値も較正できない」と単一閾値を作らなかった。ここも同じで、
   1 回の実行で 1 件だけ外れた事象から閾値を動かす根拠は細い。
2. **ギャップはあるが、較正の材料が足りない。** 0.002〜0.009 の帯に本物の
   解答が入るかどうかは、この 23 件では「数学 問3 の 0.008840 が通った」
   ことしか示せない。#122 の別実測では余白の多い計算欄が 0.015 前後に
   いる。**帯の下側を上げると、まだ見ていない教科・枠サイズで本物を弾く
   リスクを数値で否定できない。**
3. **根本原因は閾値ではなく検出枠のずれ**である（Issue #122 前半）。
   閾値を上げても、細長い切り出しにだけ効くサイズ依存の救済に留まる。

**`NEARLY_BLANK_INK_COVERAGE` は `0.002` のまま据え置く。** 変えなかった理由の
数値: 23 件中 閾値以下 0 件、人判定の空白 1 件（1.23× 閾値）、
次点との比 3.6×、面積–coverage 相関 r = −0.50（単純な面積補正では直らない）。
...
# 実機再検証 #6 の appdata + out/（リポジトリ外。比率と寸法のみ出力）
uv run python poc/issue_214_blank_crop_threshold/report.py --data /path/to/live-run
```
...
### 2ページにまたがる設問の切り出し結合（Issue #108）

1 つの設問が 2 ページにまたがる解答欄を持つ場合（`Question.page_2` と `Question.answer_area_2` が存在）、
パイプラインがそれぞれのページ（`page` および `page_2`）を順次処理する際に該当矩形を切り出し、
両ページの切り出しが得られた時点で垂直方向に結合（幅が異なる場合は白パディングで中央揃えし、
間に微小な白スペースを挿入）して 1 つの `AnswerImage` PNG 画像を生成する。
これにより、1 設問 = 1 画像という下流（AI 採点プロバイダ・OCR・レビュー画面）の契約を維持したまま、
AI に両ページの解答内容を完全に渡して正確な採点を行わせることができる。
...
- **取込に成功し、結果が `ai_processed` だった答案は、続けて
  `POST /submissions/{submission_id}/jobs` を呼んで AI 採点ジョブを起票する**
  （Issue #80。決定と理由は [job-queue.md](./job-queue.md)「起票のタイミング」）。
  `needs_review` / `error` の答案は自動では起票しない — §3 の
  「前提設問を含むページが欠落した状態では AI 採点を開始しない」をここで守る。
  起票に失敗しても取込は成功として扱い、答案は一覧に残したまま、警告バナーと
  「AI採点を開始」の再試行ボタンを出す。
...
- OCR/AI 採点 Job の起票ロジックは、`needs_review` の Submission にジョブを
  作らない制約を守って実装すること（§3）。**Issue #80 で起票側が実装され、
  自動起票を `ai_processed` に限ることでこの制約を守った**（§11）。ただし
  人間が添削レビュー画面で明示的に「AI採点を開始」を押した場合は起票する。
- **`needs_review` になった答案を回復させる道が無い。** 再取込は `error` の
  答案にしか認められず（§2 の決定表）、ページ欠落を直した PDF を取り込み直せ
  ない。Issue #80 の「AI採点を開始」は採点を始めることしかできず、取込を
  やり直すものではない。別Issueとする。
