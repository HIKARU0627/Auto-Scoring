...
ホームの入力は `listTestRegistrations` と、**全テストぶんの** `listSubmissions`
だけである（§4・§5）。それ以外は知らない。判定が「直した」のものは、その根拠を
各行のコード内コメントにも残してある。

**Issue #151 で「読んだテストの範囲の件数」が「全テストの件数」になった。**
そのため範囲を明示するために足していた3つの文字列（`（数えたのは直近N件の
テストで…）`、`直近N件のテストに、いま開く答案はありません`、`ほかにN件の
テストがあり、そちらの答案は数えていません`）は、**主張そのものが不要になったので
表から落とした**。件数の行はいずれも「全テストの件数」へ書き換えてある。
...
| `要確認の答案が${N}件あります`                                                                                                       | `needs_review` の件数                                    | **全テストの件数**（§5）                                                                                                                 | **直した**（Issue #151。8件に絞って数えていたので、溢れたテストの要確認が抜けていた）                                                                          |
| `取込済みの答案が${N}件あります`                                                                                                     | `ai_processed` の件数                                    | **全テストの件数**。採点もレビューもどこまで進んだかは知らない（§3.1）                                                                   | **直した**（review round 2 P3 の語の変更、および Issue #151 で範囲が全テストになった）                                                                         |
| `人の確認が必要と判定された答案から開きます`                                                                                         | `needs_review` は人の確認待ち                            | `state` がそう言っている（理由は `review_reason`、ホームは出さない）                                                                     | OK                                                                                                                                                             |
| `（ほかに取込済みが${N}件）`                                                                                                         | 見出し以外に残っている件数                               | **全テストの件数**                                                                                                                       | **直した**（review round 2 P3 の語の変更、および Issue #151 で範囲が全テストになった）                                                                         |
...
| `取込に失敗した答案が${N}件あります`                                                                                                 | `error` の件数                                           | **全テストの件数**。`error` は今のところ取込・finalization の失敗だけ                                                                    | **直した**（Issue #151 で範囲が全テストになった）                                                                                                              |
...
| `処理中の答案が${N}件あります`                                                                                                       | `unprocessed`/`ai_processing` の件数                     | **全テストの件数**。処理中しか無いテストはカードから落ちるが、数には残る（§2.3）                                                         | **直した**（Issue #151 で範囲が全テストになった）                                                                                                              |
| `終わると取込済み・要確認・取込失敗のいずれかになります`                                                                             | 取込の行き先                                             | 取込は3通りに落ちる（§3 の表）                                                                                                           | **直した**（1つに決めつけないこと＋語の変更）                                                                                                                  |
| `いま開く答案はありません`                                                                                                           | この画面から開く候補が無いこと                           | **全テストで**要確認も取込済みも0件であること。**レビューが済んだかどうかは言っていない**（§3.1）                                        | **直した**（review round 2 P3。旧「レビュー待ちの答案はありません」は、レビューの有無を知っているという断定だった。Issue #151 で範囲の但し書きが不要になった） |
...
| `まだテストが登録されていません`                                                                                                     | テストが1件も無いこと                                    | この分岐は `tests`（全テスト）が空。言い切ってよい唯一の不在                                                                             | OK                                                                                                                                                             |
...
| `他${hiddenTestCount}件を見る`                                               | カードに載せていないテストの数と、その行き先 | `hiddenTestCount` と `AppRoutes.testList`。**数えていないテストではない**（§2.3）                | OK                                    |
