...
`ai_processed` の答案について AI採点の話を一切できない。** `ai_processed` は
次の4つを全部覆っているからである。

1. 起票済みで、実行待ち（`QUEUED`/`BLOCKED`）
2. 起票済みで、AI処理中（`RUNNING`）
3. 起票済みで、採点は終わってレビュー待ち（`SUCCEEDED`）
4. **一度も起票されていない** — 起票が 409 や通信断で失敗した答案、および
   Issue #80 より前に取り込まれた答案（アプリからは二度と自動起票されない）

1〜3 を区別できないのは、採点ジョブが終わっても答案の `state` は `ai_processed` の
ままだからである（`needs_review` / `reviewed` へ動かすのは人間のレビュー操作の側で、
`backend/src/auto_scoring/adapters/review_actions.py` の
`_sync_submission_review_state` が担当する）。

そして **4 を 1〜3 と区別できないのが、この節の要点である。** 起票の有無は
`GET /submissions/{id}/jobs` にしか現れず、ホームはそれを引かないと決めている（§4・§5）。
4 の答案は待っても進まず、添削レビュー画面での手動起票（
[dependency-dag-progress-view.md](./dependency-dag-progress-view.md) §1.11）が要る。

したがってホームの文言は次の線を守る。

- **「採点済み」と言わない。** 3 かどうか分からない。
- **「採点中」と言わない。** 2 かどうか分からない。
- **「採点は開始済み」とも言わない。** 4 がある以上、起票されたことすら知らない。
...
bucket のラベルを「レビュー待ち」から **「採点・レビュー待ち」** へ変えたのも同じ
理由である。この bucket について言えるのは「採点もレビューもまだ済んでいない」ことだけで、
「レビュー待ち」だけだと「AIは終わっている」、「採点中」だと「動いている」の断定になる。
...
`レビュー待ちの答案について、採点が始まったとも終わったとも断定しない` がこれを固定する。
...
| ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `要確認` / `採点・レビュー待ち` / `処理中` / `取込失敗` / `確認済み`（bucket ラベル）                                                | 答案が5分類のどれか                                      | 答案の `state`。畳み方は §3 の表                                                                                                         | **直した**（Issue #80: 「レビュー待ち」はAIが終わったという断定、「採点中」は動いているという断定。どちらもホームは知らない — §3.1） |
...
| `採点・レビュー待ちの答案が${N}件あります`                                                                                           | `ai_processed` の件数                                    | 読んだテストの範囲の件数。採点が始まったかどうかも終わったかどうかも知らない（§3.1）                                                     | **直した**（範囲の但し書き、および Issue #80 の語の変更）                                                                            |
...
| `取込と回答欄の抽出は終わっています。AI採点が始まっているかどうかはホームでは分かりません。`                                         | `ai_processed` の意味                                    | 取込がそこまで終わったことは `state` が言っている。AI採点は、始まったかどうかも含めて `listJobs` を引かないと分からない（§3.1）          | **直した**（review round 1 P2-1。「AI採点は開始済み」は、起票が失敗した答案と Issue #80 より前の答案を無視した断定だった）           |
...
- **答案の `state` がAI採点の進み具合を持っていない。** 採点ジョブが全部終わっても
  `ai_processed` のままだし、そもそも起票されたかどうかも `state` には出ない
  （§3.1 の4つ）。そのためホームは「採点待ち」「採点中」「採点済みでレビュー待ち」
  「一度も起票されていない」を1つのラベルで呼んでいる。直すなら `Submission` の
  状態機械側（採点の起票・完了を状態に乗せる）であって、ホームが Job を引くことでは
