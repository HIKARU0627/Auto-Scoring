...
| `レビュー待ちの答案が${N}件あります`                                                                                                 | `ai_processed` の件数                | 同上                                                                                   | **直した**（同上）                                                              |
| `人の確認が必要と判定された答案から開きます`                                                                                         | `needs_review` は人の確認待ち        | `state` がそう言っている（理由は `review_reason`、ホームは出さない）                   | OK                                                                              |
| `（ほかにレビュー待ちが${N}件）`                                                                                                     | 見出し以外に残っている件数           | 読んだテストの範囲の件数                                                               | **直した**（範囲の但し書きを追加）                                              |
| `取込と回答欄の抽出まで終わっています。`                                                                                             | `ai_processed` の意味                | §3.1 のとおり。採点まではこの `state` では言えない                                     | OK（2回目のレビューで修正済み）                                                 |
...
| `レビュー待ちの答案はありません`                                                                                                     | 不在                                 | 全テストを読んだ場合（`hiddenTestCount == 0`）の不在                                   | OK（範囲が全部のときだけ出す）                                                  |
| `直近${tests.length}件のテストにレビュー待ちの答案はありません`                                                                      | 範囲を限った不在                     | 読んだテストの範囲の不在                                                               | **足した**（読んでいないテストがあるときは見出し自体に範囲を書く）              |
