...
- **人間がレビューを済ませた**（承認・修正・却下）。下の理由により、それでも
  答案の `state` は `ai_processed` のままである

**なぜ全部 `ai_processed` なのか。** 答案の `state` を書き換えるのは取込
（`adapters/submission_intake.py`）と `_sync_submission_review_state`
（`adapters/review_actions.py`）だけで、後者は **`needs_review` / `reviewed` の
答案にしか働かない**（それ以外は早期returnする）。採点ジョブは答案の `state` を
一切触らない。つまり取込が `ai_processed` で終わった答案は、採点が終わっても、
人間が全設問を承認しても、`ai_processed` のまま動かない。
...
- レビューが済んだかどうかも `state` からは読めない。**この bucket を
  「レビュー待ち」と呼べないのはそのためである**（§9 の未決事項）。

したがってホームの文言は次の線を守る。

- **「採点済み」と言わない。** 採点が成功したかどうか分からない。
- **「採点中」と言わない。** 動いているかどうか分からない。
- **「レビュー待ち」とも言わない。** 承認済みの答案もここに残る。
...
（Issue #80 の途中では「採点・レビュー待ち」にしていたが、**承認済みの答案もここに
残る以上「まだ済んでいない」は偽になりうる** -- review round 2, P3）。
