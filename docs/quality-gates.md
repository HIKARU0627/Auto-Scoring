...
上の 1 行は Issue #137 の担当が同じ事故に遭って書き足したものだが、**Issue #167 で
同じ事故がもう一度起きた**（変異のロールバックに `git checkout -- lib/` を使い、
未コミットだった実装を丸ごと巻き戻した。作り直してからやり直している）。
2 回起きたということは、方針として書いてあるだけでは足りないということである。
方針は急いでいるときに飛ぶ。以下をコマンドの並びとしてなぞること。
...
出力が空でないまま次へ進まない。空でなければ先にコミットする。ここが
Issue #167 で飛んだ 1 行である。

**2. 変異させるファイルを 1 つ決めて、変数に置く。**

```bash
f=backend/src/auto_scoring/domain/answer_area_detection.py
```

**3. 変異させ、落ちるはずのテストを流し、`$f` だけ戻す。**

```bash
# ここで $f を編集する（直した「性質」を壊す。値をいじるのではない）
uv run pytest tests/test_answer_area_detection.py -q
git checkout -- "$f"   # ディレクトリではなく、変異させたファイルだけ
```

`git checkout -- lib/` / `git checkout -- .` / `git checkout -- src/` は
**変異と無関係な未コミットの編集も巻き戻す**。戻すのは常に `$f` だけにする。

**4. 1 回ごとに、戻ったことを実測する。**

```bash
git status --short   # また空に戻ること
```

次の変異へ進む前に毎回見る。まとめて最後に確かめると、どの変異で汚れたか
分からなくなる。

**5. 全部終わったら、もう一度実測する。**

```bash
git status --short && git diff --stat   # どちらも空
```

#### 変異は、コンパイル/構文が通る形にする

「テストが落ちた」ではなく「**意図したテストが**落ちた」を確かめること。
Issue #167 では Dart の条件を `if (false)` に書き換えたところ `dead_code` の
解析エラーになり、テストは落ちたが**変異のせいで落ちたのではなかった**
（ファイル全体が読み込めていない）。落ちたテスト名が、狙ったものと一致して
いるかを毎回見る。分岐を殺すなら `if (false)` ではなく、その分岐ごと消すか、
返す値を別のものに差し替える。

同じ理由で、**恒真・恒偽で書き換えた「つもり」にも注意する**。Issue #164 では
`both = sorted(...)` を `both = [] or sorted(...)` に変えて「検査を外した」
つもりになったが、`[] or x` は `x` なので何も変わっておらず、テストが落ちない
のを見て一瞬「テストが空洞だ」と誤診した。変異させたら、まずその変異が
**効いていること**（狙ったテストが落ちること）を確かめる。
