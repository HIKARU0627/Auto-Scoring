...
を満たすため、`origin/main` にマージ済みの移行 PR 9 本に、Issue #273（PR #280）・Issue #271 の PR（#283）・PR #279・本 PR #282 を加えた 13 本の引き取り実績を GitHub App API 経由で取得し、§16 の割当表 185 件（`INV-*` 170 件 + `UG-*` 15 件）と照合した。
...
| **本 PR #282** | デザイントークン・アクセシビリティ（Issue #270） |            **7** | `INV-080`〜`INV-086`                                                                                                  | `desktop/test/design-tokens-lint.test.ts`・`typography.test.ts`・`design-tokens-screens.test.tsx` で固定          |
| **合計**       | (13 PR)                                          |          **128** | (ユニーク 128 件)                                                                                                     | 重複引き取りなし                                                                                                  |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）および Issue #268 が引き取った 4 件（`INV-012`, `INV-013`, `INV-016`, `INV-017`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み 135 件（128 + 3 + 4）に含める。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                                   |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **135** |  73.0% | PR #225, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283 および Issue #267, #268 により新スタックで固定済み |
| **未引き取り**   |  **50** |  27.0% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                                             |
...
### 17.3 未引き取り 50 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 50 件」の間には **30 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
2. **共通基盤側の未引き取り（29 件、PR #279・#282 反映後）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #280（6 件: `INV-200`〜`205`）、PR #283（2 件: `INV-004`, `INV-101`）、Issue #268（4 件: `INV-012`, `INV-013`, `INV-016`, `INV-017`）、PR #279（1 件: `UG-05`）、本 PR #282（7 件: `INV-080`〜`INV-086`）が引き取った 45 件を除く **29 件**が依然として未引き取りである。
...
**結論:** cut-over 条件である「185 件の不変条件を覆うこと」を満たすには、残る未移植 3 画面（答案確定・設定・出力）とテスト設定プロファイルの移植だけでは不足であり、**共通基盤の未引き取り 29 件を固定する独立した Issue / テスト整備が必要**である。

### 17.4 不変条件 185 件の完全照合一覧表

§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283）および Issue #267, #268 の引き取り実績と突き合わせた完全照合表を以下に示す。

- **引き取り済み:** 計 **135 件**（新スタックのテストで固定済み）
- **未引き取り:** 計 **50 件**（未移植画面 21 件 + 未引き取り共通基盤 29 件）
- **二重計上:** **0 件**（主担当としての重複引き取りなし）
- **合計件数:** **185 件（135 + 50 = 185、完全一致）**
...
> 補足（Issue #273 / PR #280）: 本表が分類した 52 件のうち、分類 C の API クライアント層 6 件（`INV-200`〜`INV-205`）は PR #280 で引き取り済みとなった。§17 の集計は引き取り済み 135・未引き取り 50 に更新されている。
