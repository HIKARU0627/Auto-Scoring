...
| 状態区分         |    件数 |   割合 | 概要                                                                                                         |
| ---------------- | ------: | -----: | ------------------------------------------------------------------------------------------------------------ |
| **引き取り済み** | **122** |  65.9% | マージ済み PR #225, #235, #237, #238, #240, #243, #244 および Issue #267, PR #270 により新スタックで固定済み |
...
└─ 共通基盤側の残余: 42 件
    ...
    ├─ デザイントークン・アクセシビリティ: 1 件（INV-095） ★ PR #270 にて INV-080〜086 の 7 件を引き取り
...
2. **共通基盤側の未引き取り（42 件）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #270（7 件: `INV-080`〜`INV-086`）が引き取った 32 件を除く **42 件**が依然として未引き取りである。
...
§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #277, #270）の引き取り実績と突き合わせた完全照合表を以下に示す。
...
| INV-080    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | `lib/features/` に padding/gap/radius/elevation/icon/layout/色リテラル禁止         |
| INV-081    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | 主要 7 画面が light/dark 両テーマで surface ramp から背景                          |
| INV-082    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | Noto Sans JP variable（fvar、≥17000 glyphs）                                       |
| INV-083    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | 数字は等幅（点数更新で横ジャンプしない）                                           |
| INV-084    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | fontWeight が variable axis を駆動                                                 |
| INV-085    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | 全 text role が bundled family + height                                            |
| INV-086    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #270                                                                   | 認識文字 role は body より大・letterSpacing>0                                      |
...
| **INV-080**    | トークン       | **C** | `desktop/test/design-tokens-lint.test.ts`                                                                                                           | `desktop/src/renderer/features/` 配下のスタイル・色リテラル直書き禁止静的検査（PR #270 で引き取り完了）                                                                                       |
| **INV-081**    | トークン       | **C** | `desktop/test/renderer/design-tokens-screens.test.tsx`                                                                                              | 主要画面ルート要素の light/dark テーマ surface ramp 背景色適用検証（PR #270 で引き取り完了）                                                                                                  |
| **INV-082**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | Noto Sans JP フォントのグリフ数（≥17000）および可変軸検証（PR #270 で引き取り完了）                                                                                                           |
| **INV-083**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | 数字表示スタイルの等幅（`tabular-nums`）検証（PR #270 で引き取り完了）                                                                                                                        |
| **INV-084**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | `fontWeight` の指定による可変フォント軸駆動検証（PR #270 で引き取り完了）                                                                                                                     |
| **INV-085**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | 全テキストロールの font-family および line-height 網羅検証（PR #270 で引き取り完了）                                                                                                          |
| **INV-086**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | 認識文字ロールのフォントサイズおよび字間設定検証（PR #270 で引き取り完了）                                                                                                                    |
...
#### 5. デザイントークン・アクセシビリティ（7 件中 7 件引き取り完了 ★ PR #270）

- **`INV-080` (スタイルリテラル禁止)**: `desktop/test/design-tokens-lint.test.ts` にて `features/` 配下のスタイル・色リテラル直書きを禁止する静的検査テストを実装（PR #270 で引き取り完了）。
- **`INV-081` (主要画面の surface ramp 背景色)**: `desktop/test/renderer/design-tokens-screens.test.tsx` にて各画面ルート要素の背景色がトークン由来であることを検証（PR #270 で引き取り完了）。
- **`INV-082` (Noto Sans JP variable グリフ数)**: `desktop/test/typography.test.ts` にて配布フォントのグリフ数（≥17000）および可変軸（`fvar`）を検証（PR #270 で引き取り完了）。
- **`INV-083` (数字の等幅設定)**: `desktop/test/typography.test.ts` にて点数等の数字表示スタイルで `tabular-nums` が指定されていることを検証（PR #270 で引き取り完了）。
- **`INV-084` (fontWeight の可変軸駆動)**: `desktop/test/typography.test.ts` にて可変フォントのウェイト軸駆動を検証（PR #270 で引き取り完了）。
- **`INV-085` (全 text role の bundled family + height)**: `desktop/test/typography.test.ts` にて全テキストロールの font-family / line-height 設定を検証（PR #270 で引き取り完了）。
- **`INV-086` (認識文字 role の寸法・字間)**: `desktop/test/typography.test.ts` にて認識文字ロールのフォントサイズ（> 16px）および字間（letter-spacing > 0）を検証（PR #270 で引き取り完了）。
