...
| 状態区分         |    件数 |   割合 | 概要                                                                                    |
| ---------------- | ------: | -----: | --------------------------------------------------------------------------------------- |
| **引き取り済み** | **113** |  61.1% | マージ済み PR #225, #235, #237, #238, #240, #243, #244, #279 により新スタックで固定済み |
| **未引き取り**   |  **72** |  38.9% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤              |
...
### 17.3 未引き取り 72 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 72 件」の間には **52 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
└─ 共通基盤側の残余: 51 件（PR #279 で UG-05 引き取り後）
...
2. **共通基盤側の未引き取り（51 件、PR #279 反映後）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、PR #279（1 件: `UG-05`）が引き取った 23 件を除く **51 件**が依然として未引き取りである。
...
- **未引き取り:** 計 **72 件**（未移植画面 21 件 + 未引き取り共通基盤 51 件）
