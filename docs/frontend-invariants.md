...
| **合計** | (16 PR)                                          |          **141** | (ユニーク 141 件)                                                                                                     | 重複引き取りなし                                                                                                                                       |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）、Issue #268 が引き取った 4 件（`INV-012`, `INV-013`, `INV-016`, `INV-017`）、および Issue #192 が引き取った 2 件（`INV-015`, `INV-019`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み **150 件（PR 単位 141 + Issue 名義 3 + 4 + 2）** に含める。この等式（`§17.2 の引き取り済み = 本表の合計 + Issue 名義の引き取り`）は `desktop/test/frontend-invariants-tally.test.ts` が検査する（§17.0 参照）。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                                                           |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **150** |  81.1% | PR #225, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283, #291, #305, #308 および Issue #267, #268, #192 により新スタックで固定済み |
| **未引き取り**   |  **35** |  18.9% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                                                                     |
...
### 17.3 未引き取り 35 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 35 件」の間には **15 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
└─ 共通基盤側の残余: 14 件（PR #279・#282・#305・#308・本 Issue #266・Issue #192 反映後）
    ├─ ナビゲーション・ルーティング: 3 件（INV-010, INV-011, INV-020）
    ├─ 共通 UI コンポーネント: 2 件（INV-091, INV-092） ★ INV-096〜INV-168 の 8 件は PR #308 で引き取り済み
    ├─ デザイントークン・アクセシビリティ: 1 件（INV-095） ★ PR #282 にて INV-080〜086 の 7 件を引き取り
    ├─ サイドカーライフサイクル・起動ゲート: 3 件（UG-02〜04） ※ PR #305 にて INV-033, UG-14, UG-15 の 3 件を引き取り
    ├─ API クライアント: 0 件（INV-200〜205 は PR #280 で引き取り済み）
    ├─ 操作要件・無効理由エンジン: 3 件（INV-201-04, INV-100, INV-102）
    ├─ アーキテクチャ・依存方向: 0 件（INV-001, INV-002 は本 Issue #266 で引き取り済み）
...
2. **共通基盤側の未引き取り（14 件、PR #279・#282・#305・#308・本 Issue #266・Issue #192 反映後）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #280（6 件: `INV-200`〜`205`）、PR #283（2 件: `INV-004`, `INV-101`）、Issue #268（4 件: `INV-012`, `INV-013`, `INV-016`, `INV-017`）、PR #279（1 件: `UG-05`）、PR #282（7 件: `INV-080`〜`INV-086`）、本 Issue #266（2 件: `INV-001`, `INV-002`）、Issue #192（2 件: `INV-015`, `INV-019`）、PR #305（3 件: `INV-033`, `UG-14`, `UG-15`）、PR #308（8 件: `INV-096`, `INV-097`, `INV-104`, `INV-105`, `INV-106`, `INV-108`, `INV-109`, `INV-168`）が引き取った 60 件を除く **14 件**が依然として未引き取りである。
   - 画面移植（Phase 3）は各画面のローカルな振る舞いを引き取るが、アプリ全体のルーター規約（`INV-010`〜`013` 等）、共通ボタン・エラーバナー等の共通 UI（`INV-091`〜`109`）、無効理由エンジン（`INV-100`〜`102`）などの横断的不変条件は画面移植のスコープから自然と外れるためである。API クライアント通信規約（`INV-200`〜`205`）は PR #280 が引き取った。

**結論:** cut-over 条件である「185 件の不変条件を覆うこと」を満たすには、残る未移植 3 画面（答案確定・設定・出力）とテスト設定プロファイルの移植だけでは不足であり、**共通基盤の未引き取り 14 件を固定する独立した Issue / テスト整備が必要**である。

### 17.4 不変条件 185 件の完全照合一覧表

§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283, #291, #305, #308）および Issue #267, #268 の引き取り実績と突き合わせた完全照合表を以下に示す。
...
> 補足（Issue #273 / PR #280）: 本表が分類した 52 件のうち、分類 C の API クライアント層 6 件（`INV-200`〜`INV-205`）は PR #280 で引き取り済みとなった。§17 の集計は引き取り済み 150・未引き取り 35 に更新されている。
