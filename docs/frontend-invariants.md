...
を満たすため、`origin/main` にマージ済みの移行 PR 9 本に、Issue #273（PR #280）・Issue #271 の PR（#283）・PR #279・PR #282・本 Issue #266 の PR を加えた 14 本の引き取り実績を GitHub App API 経由で取得し、§16 の割当表 185 件（`INV-*` 170 件 + `UG-*` 15 件）と照合した。
...
| **本 PR** | アーキテクチャ・依存方向（C-1 / Issue #266）     |            **2** | `INV-001`, `INV-002`                                                                                                  | `desktop/test/architecture.test.ts` の sub-layer ルール。違反していた 6 モジュールを `core/` へ移し、#255 管轄の 1 件のみ厳密 allowlist |
| **合計**  | (14 PR)                                          |          **130** | (ユニーク 130 件)                                                                                                     | 重複引き取りなし                                                                                                                        |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）および Issue #268 が引き取った 4 件（`INV-012`, `INV-013`, `INV-016`, `INV-017`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み 137 件（130 + 3 + 4）に含める。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                                                        |
| ---------------- | ------: | -----: | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **137** |  74.1% | PR #225, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283 および Issue #267, #268・本 Issue #266 の PR により新スタックで固定済み |
...
§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283）および Issue #267, #268・本 Issue #266 の PR の引き取り実績と突き合わせた完全照合表を以下に示す。

- **引き取り済み:** 計 **137 件**（新スタックのテストで固定済み）
- **未引き取り:** 計 **48 件**（未移植画面 21 件 + 未引き取り共通基盤 27 件）
- **二重計上:** **0 件**（主担当としての重複引き取りなし）
- **合計件数:** **185 件（137 + 48 = 185、完全一致）**
...
| INV-001    | 共通基盤     | 共通基盤: アーキテクチャ・依存方向                                     | **引き取り済み** | 本 PR（`desktop/test/architecture.test.ts`）                              | `api` 層は `core` / `features` を import しない                                    |
| INV-002    | 共通基盤     | 共通基盤: アーキテクチャ・依存方向                                     | **引き取り済み** | 本 PR（`desktop/test/architecture.test.ts`）                              | `core` 層は `features` を import しない                                            |
| INV-003    | 画面         | 添削レビュー                                                           | **引き取り済み** | PR #244                                                                   | 添削レビュー画面は `setState` 直呼び禁止。`_setStateIfMounted` 経由のみ            |
| INV-004    | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | 本 PR（`desktop/test/action-requirements-invariants.test.ts`）            | 無効操作の判定と文言は `core/action_requirements.dart` の単一ソース                |
...
| INV-101    | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | 本 PR（`desktop/test/action-requirements-invariants.test.ts`）            | 理由文は「。」終わり・Exception/HTTP/数字なし                                      |
