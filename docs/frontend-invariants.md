...
| 状態区分         |    件数 |   割合 | 概要                                                                                                      |
| ---------------- | ------: | -----: | --------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **119** |  64.3% | マージ済み PR #225, #235, #237, #238, #240, #243, #244 および Issue #267, #268 により新スタックで固定済み |
| **未引き取り**   |  **66** |  35.7% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                                |
...
### 17.3 未引き取り 66 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 66 件」の間には **46 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
2. **共通基盤側の未引き取り（45 件）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、Issue #268（4 件: `INV-012`, `INV-013`, `INV-016`, `INV-017`）が引き取った 29 件を除く **45 件**が依然として未引き取りである。
