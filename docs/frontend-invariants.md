...
| 状態区分         |    件数 |   割合 | 概要                                                                                           |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------- |
| **引き取り済み** | **121** |  65.4% | PR #225, #235, #237, #238, #240, #243, #244, #280 および Issue #267 により新スタックで固定済み |
| **未引き取り**   |  **64** |  34.6% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                     |
...
### 17.3 未引き取り 64 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 64 件」の間には **44 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
2. **共通基盤側の未引き取り（43 件）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #280（6 件: `INV-200`〜`205`）が引き取った 31 件を除く **43 件**が依然として未引き取りである。
...
**結論:** cut-over 条件である「185 件の不変条件を覆うこと」を満たすには、残る未移植 3 画面（答案確定・設定・出力）とテスト設定プロファイルの移植だけでは不足であり、**共通基盤の未引き取り 43 件を固定する独立した Issue / テスト整備が必要**である。
...
- **引き取り済み:** 計 **121 件**（新スタックのテストで固定済み）
- **未引き取り:** 計 **64 件**（未移植画面 21 件 + 未引き取り共通基盤 43 件）
- **二重計上:** **0 件**（主担当としての重複引き取りなし）
- **合計件数:** **185 件（121 + 64 = 185、完全一致）**
