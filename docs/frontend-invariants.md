...
**Flutter 側の除外（技術的障害なし）:** Flutter の `home_escape_test.dart:39` は `submissionQueue` / `pdfReview` を意図的に除外している。Electron 版 `desktop/test/renderer/home-escape.test.tsx` は両ルートとも `covered` として検査対象に入れており、Issue #192 で `pdfReview` の最後の空白を解消した。
...
新スタックでの固定先は `desktop/test/action-requirements-invariants.test.ts` である。

**INV-101（文言規約）** は `ActionRequirements` の全メンバーを実行時に列挙し、各理由文が
「。」で終わること、例外名（`Exception` / `...Error`）や HTTP ステータスコード
（3 桁の数字 + 空白）を含まないことを検査する。メンバーの追加は列挙に自動で乗るため、
規約を破った理由を足した時点で赤くなる。

**INV-004（単一ソース性）** は `src/renderer/features/` 配下を走査し、画面が
`message` / `action` の prop へ直接書いた「。」終わりの文字列リテラルを検出する。
対象は **「その操作がいま使えない理由」を説明する文言** に限る。範囲の線引きは次のとおり。

- **対象に含める**: 画面が確定・取込などの可否理由を出すときの通り道である
  `message` / `action` の prop に直接書かれた理由文。
- **対象から外す: フォーム入力のバリデーション**（`setError(...)` 等）。項目単位の
  入力エラーであって操作の無効理由ではなく、INV-108 が helperText と disabled reason を
  別物として扱っていることが根拠である。
- **対象から外す: 起動オーバーレイ・接続状態の説明**。サイドカーのライフサイクル状態の
  説明であり、UG-14 / UG-15 の担当範囲である。
- **対象から外す: 操作の結果通知**。`message={ outcome === null ? "..." : "..." }` の
  ように値が式の結果であり、操作が使えない理由ではなく操作の結果だからである。
- **対象から外す: JSX 直下のテキストノード**。文字列リテラルではないため静的走査の
  対象ではなく、該当画面の振る舞いテスト（例: INV-067）が担当する。

既存の違反は #255 が `features/` を編集中のため本 Issue では直さず、
`OUTSTANDING_REASONS`（ファイル + コンポーネント + 文面の同一性で 10 件）として明示的に
...
を満たすため、`origin/main` にマージ済みの移行 PR 9 本に、Issue #273（PR #280）・Issue #271 の PR（#283）・PR #279・PR #282・Issue #266 の PR（#291）を加えた 14 本の引き取り実績を GitHub App API 経由で取得し、§16 の割当表 185 件（`INV-*` 170 件 + `UG-*` 15 件）と照合した。

### 17.0 集計は §17.4 の行から導出する（正本と検査）

この節と §18 に出てくる件数（引き取り済み／未引き取り、PR 別の宣言件数、未引き取りの内訳、分類 A/B/C）は、
**§17.4 の完全照合一覧表「状態」列を唯一の真実 (single source of truth) として数え直した結果**である。
個々の不変条件の行が「**引き取り済み**」「未引き取り」を持ち、§17.1（PR 別証跡）・§17.2（状態サマリ）・
§17.3（未引き取り内訳）・§18（共通基盤の分類）がそれを再掲する。

- **行 → 集計の突き合わせは機械が行う:** `desktop/test/frontend-invariants-tally.test.ts` が §17.4 の行を
  パースして集計し、文書中の各集計と一致することを検査する。**集計だけを直して行を直し忘れた場合も、
  行だけを直して集計を放置した場合も、どちらも赤くなる。**このファイルが、この文書で唯一数値を
  手で合わせる必要がある PR の衝突点を検出する。
- **§17.1 と §17.2 の差の規則:** PR 単位の引き取りは §17.1 の証跡表に載せ、PR 単位でない引き取り
  （Issue #267・#268 のような Issue 名義のもの）は §17.1 には載せず §17.2 の引き取り済みに含める。
  したがって `§17.2 の引き取り済み = §17.1 の合計（PR 単位） + Issue 名義の引き取り` が常に成り立つ。
  テストはこの等式を検査するので、**PR 単位でない引き取りが増えても差が説明可能なまま**である。
- **行を増減させるとき:** 合計は **185 件（INV 170 件 + UG 15 件）** で固定する。行を足す／削ると
  §17.4 の合計と §16.4 の合計が同時に赤くなるため、意図した変更ならテストの指示に従って
  §16.4・§17・§18 の各集計も更新する。
- **変異検査（Issue #290 で実施）:** 集計の数字を 1 つ書き換える・行の「状態」を 1 つ書き換える・
  行を 1 つ削って合計を 184 にする、のいずれでもテストが赤くなることを確認した。
...
| **#279** | 登録完了 UI + UG-05（E2E 環境変数ゲート）           |            **1** | `UG-05`                                                                                                                                       | `desktop/src/main/e2e-env.ts` + `desktop/test/e2e-env.test.ts`。パッケージ版では E2E 環境変数を無視                                                                             |
| **#280** | API クライアント層（C-8 / Issue #273）              |            **6** | `INV-200`〜`INV-205`                                                                                                                          | main の `sidecarFetch` / `sidecar-connection` で token 認証・非 loopback 拒否・秘匿・409 を固定                                                                                 |
| **#283** | 操作要件・無効理由エンジン（検査）                  |            **2** | `INV-004`, `INV-101`                                                                                                                          | `desktop/test/action-requirements-invariants.test.ts`。単一ソース性の静的走査と文言規約の全件列挙                                                                               |
| **#282** | デザイントークン・アクセシビリティ（Issue #270）    |            **7** | `INV-080`〜`INV-086`                                                                                                                          | `desktop/test/design-tokens-lint.test.ts`・`typography.test.ts`・`design-tokens-screens.test.tsx` で固定                                                                        |
| **#291** | アーキテクチャ・依存方向（C-1 / Issue #266）        |            **2** | `INV-001`, `INV-002`                                                                                                                          | `desktop/test/architecture.test.ts` の sub-layer ルール。違反していた 6 モジュールを `core/` へ移し、#255 管轄の 1 件のみ厳密 allowlist                                         |
| **#305** | サイドカー起動ゲート補完（Issue #269）              |            **3** | `INV-033`, `UG-14`, `UG-15`                                                                                                                   | push 済み画面のクラッシュ overlay・ログ位置・待つ理由の案内を `desktop/test/renderer/sidecar-startup-overlay.test.tsx` で固定                                                   |
| **#308** | 共通 UI コンポーネント 8 件（C-7 / Issue #272）     |            **8** | `INV-096`, `INV-097`, `INV-104`, `INV-105`, `INV-106`, `INV-108`, `INV-109`, `INV-168`                                                        | AppErrorBanner・QuestionStatusBadge・GradingUnavailableBanner を core に追加し DisabledActionReason を表示専用へ。renderer テスト 4 本 16 tests で固定                          |
| **#323** | 出力機能 12 件（Issue #321）                        |           **12** | `INV-180`〜`INV-190`, `UG-09`                                                                                                                 | 出力画面は Issue #250 / PR #251 で移植済み。12 件すべての変異検査で約束を固定（`desktop/test/renderer/export-dialog.test.tsx`・`desktop/test/bulk-export-writer.test.ts` ほか） |
| **#324** | 共通基盤の未引き取り 14 件（Issue #322）            |           **14** | `INV-010`, `INV-011`, `INV-020`, `INV-091`, `INV-092`, `INV-095`, `UG-02`〜`UG-04`, `INV-201-04`, `INV-100`, `INV-102`, `INV-201-07`, `UG-13` | 分類 A 7 件は走査ガード、分類 B 7 件は明示テストで固定（`desktop/test/routing-invariants.test.ts`・`theme-invariants.test.ts`・`sidecar-invariants.test.ts` ほか）              |
| **#326** | 答案確定・設定・プロファイル確定 9 件（Issue #320） |            **9** | `INV-201-02`, `INV-201-08`, `INV-005`, `INV-154`〜`INV-157`, `INV-107`, `INV-110`                                                             | 未移植 3 画面のローカル不変条件を既存実装へのテストで固定（`desktop/test/submission-confirmation.test.ts` ほか）                                                                |
| **合計** | (19 PR)                                             |          **176** | (ユニーク 176 件)                                                                                                                             | 重複引き取りなし                                                                                                                                                                |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）、Issue #268 が引き取った 4 件（`INV-012`, `INV-013`, `INV-016`, `INV-017`）、および Issue #192 が引き取った 2 件（`INV-015`, `INV-019`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み **185 件（PR 単位 176 + Issue 名義 3 + 4 + 2）** に含める。この等式（`§17.2 の引き取り済み = 本表の合計 + Issue 名義の引き取り`）は `desktop/test/frontend-invariants-tally.test.ts` が検査する（§17.0 参照）。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                                                                             |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **185** | 100.0% | PR #225, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283, #291, #305, #308, #323, #324, #326 および Issue #267, #268, #192 により新スタックで固定済み |
| **未引き取り**   |   **0** |   0.0% | なし（185 件すべてが引き取り済み）                                                                                                                               |
...
### 17.3 未引き取り 0 件の確認（旧: 35 件の内訳と未移植画面との差分分析）

§17.4 の全 185 行が「引き取り済み」になったため、未引き取り全 0 件・画面側の残余: 0 件・共通基盤側の残余: 0 件である。旧版にあった未引き取り 35 件の内訳木は、対象が消えたため削除した。

この 35 件は、次の 3 本の PR が引き取った。

- **PR #323（12 件・画面）**: 出力機能の `INV-180`〜`INV-190` と `UG-09`（Issue #321）。
- **PR #324（14 件・共通基盤）**: `INV-010`, `INV-011`, `INV-020`, `INV-091`, `INV-092`, `INV-095`, `UG-02`〜`UG-04`, `INV-201-04`, `INV-100`, `INV-102`, `INV-201-07`, `UG-13`（Issue #322）。§18 の分類で A（構造的に起こりえない）7 件と B（既存テストが覆う）7 件だった残余を、走査ガードと明示テストで実行可能な形に固定した。
- **PR #326（9 件・画面）**: 答案確定 7 件（`INV-201-02`, `INV-201-08`, `INV-005`, `INV-154`〜`INV-157`）・設定 1 件（`INV-107`）・テスト設定プロファイル確定 1 件（`INV-110`）（Issue #320）。

旧版では「未移植 3 画面（答案確定・設定・出力）の 20 件」と未引き取り 35 件の間に差分 15 件があり、その実態は画面側の積み残し（`INV-110`）と共通基盤側の横断的不変条件（14 件）だった。この差分は上記 3 本で全部が引き取られ、いまや 0 件の差である。cut-over 条件「185 件の不変条件を覆うこと」は、§17.4 の全行が引き取り済みであることと一致する。

### 17.4 不変条件 185 件の完全照合一覧表

§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283, #291, #305, #308, #323, #324, #326）および Issue #267, #268 の引き取り実績と突き合わせた完全照合表を以下に示す。

**この表の「状態」列が引き取り状況の唯一の真実 (single source of truth) である。** §16.4 の割当内訳・§17.1 の PR 別証跡・§17.2 の状態サマリ（引き取り済み／未引き取り／合計）・§17.3 の未引き取り内訳・§18 の分類件数は、すべてこの「状態」列と §16.5 の「区分」列から導出され、`desktop/test/frontend-invariants-tally.test.ts` が一致を検査する（§17.0 参照）。件数の要約は §17.2 にある。

| ID         | 区分         | 割当担当先                                                             | 状態             | 引き取り元 PR / 残存理由                                         | 不変条件 / 保証の概要                                                              |
| ---------- | ------------ | ---------------------------------------------------------------------- | ---------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| INV-201-01 | 画面         | 添削レビュー（主）/ 答案確定（副）                                     | **引き取り済み** | PR #244                                                          | 見ていない判断材料で確定・承認できない                                             |
| INV-201-02 | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | 確信度（Confidence）から完了・確定を導かない                                       |
| INV-201-03 | 画面         | 添削レビュー                                                           | **引き取り済み** | PR #244                                                          | 失敗が要約から漏れない。パネルを畳んでも残る。下流の「待ち」と区別できる           |
| INV-201-04 | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | PR #324                                                          | 無効な操作には、無効条件そのものから導いた理由が必ず出る                           |
...
| INV-201-07 | 共通基盤     | 共通基盤: 全体検証・文言検査（関連: ホーム、取込）                     | **引き取り済み** | PR #324                                                          | 廃止した文言が二度と現れない                                                       |
| INV-201-08 | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | 別ボタンにフォーカスした Enter が確定を実行しない                                  |
...
| INV-001    | 共通基盤     | 共通基盤: アーキテクチャ・依存方向                                     | **引き取り済み** | PR #291（`desktop/test/architecture.test.ts`）                   | `api` 層は `core` / `features` を import しない                                    |
| INV-002    | 共通基盤     | 共通基盤: アーキテクチャ・依存方向                                     | **引き取り済み** | PR #291（`desktop/test/architecture.test.ts`）                   | `core` 層は `features` を import しない                                            |
| INV-003    | 画面         | 添削レビュー                                                           | **引き取り済み** | PR #244                                                          | 添削レビュー画面は `setState` 直呼び禁止。`_setStateIfMounted` 経由のみ            |
| INV-004    | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | PR #283（`desktop/test/action-requirements-invariants.test.ts`） | 無効操作の判定と文言は `core/action_requirements.dart` の単一ソース                |
| INV-005    | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | 答案確定可否は `core/submission_confirmation.dart` に置く                          |
| INV-006    | 共通基盤     | 共通基盤: ドメインロジック・設問状態（関連: 添削レビュー、答案キュー） | **引き取り済み** | Issue #267                                                       | 設問ステータスは `core/question-status.ts` の `deriveQuestionStatus` で一元決定    |
| INV-007    | 画面         | 答案キュー（関連: ホーム）                                             | **引き取り済み** | PR #243                                                          | レビューキュー順序は `core/review_queue.dart` で決定                               |
| INV-010    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | PR #324                                                          | `features/` 内で `go_router` の `.go()` / `.goNamed()` を使わない                  |
| INV-011    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | PR #324                                                          | 画面間移動は `push` / `replace` / `pop` のみ                                       |
| INV-012    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | Issue #268                                                       | `AppRoutes` の各 location が対応 Widget を開く                                     |
| INV-013    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | Issue #268                                                       | パラメータ付き URL（`testId`, `submissionId`, `questionId`）が正しく decode される |
| INV-014    | 画面         | 添削レビュー                                                           | **引き取り済み** | PR #244                                                          | 添削レビューは開く設問を URL で指定できる                                          |
| INV-015    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | Issue #192                                                       | スタック空で開いた covered ルートすべてに `BackOrHomeButton` がある                |
| INV-016    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | Issue #268                                                       | 狭幅 700×720 でも出口ボタンが押せる                                                |
| INV-017    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | Issue #268                                                       | Tab→Enter のみでホームへ戻れる                                                     |
| INV-018    | 画面         | ホーム（Phase 2-4）                                                    | **引き取り済み** | PR #235                                                          | ホーム自身には出口を出さない                                                       |
| INV-019    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | Issue #192                                                       | push で積んだ画面では出口は 1 枚戻る（ホーム直行ではない）                         |
| INV-020    | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | PR #324                                                          | 新ルート追加時、covered か skipped（理由付き）のどちらかに必ず登録                 |
...
| INV-033    | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #305                                                          | push 済み画面の上でもクラッシュ overlay＋再起動が見える                            |
...
| INV-080    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | `lib/features/` に padding/gap/radius/elevation/icon/layout/色リテラル禁止         |
| INV-081    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | 主要 7 画面が light/dark 両テーマで surface ramp から背景                          |
| INV-082    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | Noto Sans JP variable（fvar、≥17000 glyphs）                                       |
| INV-083    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | 数字は等幅（点数更新で横ジャンプしない）                                           |
| INV-084    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | fontWeight が variable axis を駆動                                                 |
| INV-085    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | 全 text role が bundled family + height                                            |
| INV-086    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #282                                                          | 認識文字 role は body より大・letterSpacing>0                                      |
...
| INV-091    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #324                                                          | Material 既定 disabled outline は AA 未達（上書き理由の実測）                      |
| INV-092    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #324                                                          | filled/outlined/text 全 ButtonTheme が disabled 色を返す                           |
...
| INV-095    | 共通基盤     | 共通基盤: デザイントークン・アクセシビリティ                           | **引き取り済み** | PR #324                                                          | AppStatusColors / AppTextRoles extension 必須                                      |
| INV-096    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #308                                                          | AppErrorBanner: 再試行・busy 時 disable・1 回 fade-in で settle                    |
| INV-097    | 共通基盤     | 共通基盤: 共通UIコンポーネント（関連: 添削レビュー、答案キュー）       | **引き取り済み** | PR #308                                                          | QuestionStatus: 各状態に固有 label+icon（色だけにしない）                          |
| INV-100    | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | PR #324                                                          | 無効操作には理由 id が 1 件以上、有効時 0 件                                       |
| INV-101    | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | PR #283（`desktop/test/action-requirements-invariants.test.ts`） | 理由文は「。」終わり・Exception/HTTP/数字なし                                      |
| INV-102    | 共通基盤     | 共通基盤: 操作要件・無効理由エンジン                                   | **引き取り済み** | PR #324                                                          | busy は単独でも理由になる                                                          |
| INV-103    | 画面         | 取込                                                                   | **引き取り済み** | PR #240                                                          | intake: template 未選択→`intake-template`                                          |
| INV-104    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #308                                                          | 条件充足で理由消去＋高さ 0（余白残さない）                                         |
| INV-105    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #308                                                          | 理由は Tooltip ではなく visible Text                                               |
| INV-106    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #308                                                          | 狭幅 700×720 で理由が画面内                                                        |
| INV-107    | 画面         | 設定                                                                   | **引き取り済み** | PR #326                                                          | API キー: 未設定→verify 無効+理由                                                  |
| INV-108    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #308                                                          | helperText と disabled reason の二重表示禁止                                       |
| INV-109    | 共通基盤     | 共通基盤: 共通UIコンポーネント                                         | **引き取り済み** | PR #308                                                          | Tab 順: 理由 Text は focus 取らず隣ボタン到達可能                                  |
| INV-110    | 画面         | テスト設定（Phase 3 テスト設定）                                       | **引き取り済み** | PR #326                                                          | profile confirm 理由 ⊇ save 理由（部分集合）                                       |
...
| INV-152    | 共通基盤     | 共通基盤: ドメインロジック・設問状態（関連: 添削レビュー）             | **引き取り済み** | Issue #267                                                       | usable=false + waiting → needsCheck; 待ち無し → graded                             |
| INV-153    | 共通基盤     | 共通基盤: ドメインロジック・設問状態（関連: 添削レビュー）             | **引き取り済み** | Issue #267                                                       | 人間採点後は stopped job より review 優先                                          |
| INV-154    | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | 未到達設問があれば確定不可、番号明示                                               |
| INV-155    | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | 全到達後 1 操作で N 問 approve                                                     |
| INV-156    | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | 部分失敗: 確定済み数・残り・failedNumber 表示                                      |
| INV-157    | 画面         | 答案確定                                                               | **引き取り済み** | PR #326                                                          | AI 採点不可設問→humanScoreRequired で確定不可                                      |
...
| INV-168    | 共通基盤     | 共通基盤: 共通UIコンポーネント（アプリシェル）                         | **引き取り済み** | PR #308                                                          | grading unavailable banner: unavailable のみ表示                                   |
...
| INV-180    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 単体: progress→success + path                                                      |
| INV-181    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | `reuse_existing` は即 success                                                      |
| INV-182    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 409 `unconfirmed_questions`→設問 id 一覧                                           |
| INV-183    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 409 `no_room_for_score`≠未確認文言                                                 |
| INV-184    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 未知 conflict code→sidecar message をそのまま                                      |
| INV-185    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | failed→retry で requeue                                                            |
| INV-186    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | ポーリング transient failure は progress 維持                                      |
| INV-187    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | cancelled job retry→新規 requestExport                                             |
| INV-188    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 一括: 全問確定のみ対象、除外は理由付き                                             |
| INV-189    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 1 件 refused でも残り続行、失敗一覧                                                |
| INV-190    | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | キュー行から export（レビュー画面経由不要）                                        |
| INV-200    | 共通基盤     | 共通基盤: API クライアント                                             | **引き取り済み** | PR #280                                                          | `/healthz` は bogus token でも OK                                                  |
| INV-201    | 共通基盤     | 共通基盤: API クライアント                                             | **引き取り済み** | PR #280                                                          | 正 token で protected API 成功                                                     |
| INV-202    | 共通基盤     | 共通基盤: API クライアント                                             | **引き取り済み** | PR #280                                                          | 誤 token→401 unauthorized                                                          |
| INV-203    | 共通基盤     | 共通基盤: API クライアント                                             | **引き取り済み** | PR #280                                                          | 非 loopback URL→拒否（URL/token を message に含めない）                            |
| INV-204    | 共通基盤     | 共通基盤: API クライアント                                             | **引き取り済み** | PR #280                                                          | unknown transport error→接続詳細なし                                               |
| INV-205    | 共通基盤     | 共通基盤: API クライアント                                             | **引き取り済み** | PR #280                                                          | 409→`conflictCode` + `conflictQuestionIds` 保持                                    |
| UG-01      | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #237                                                          | アプリを強制終了してもサイドカーが道連れで終わる（Job Object kill-on-close）       |
| UG-02      | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #324                                                          | kill-on-close を設定できなかった Job は持たない（保証が有るふりをしない）          |
| UG-03      | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #324                                                          | Job ハンドルはプロセスの生涯閉じない（`dispose()` でも閉じない）                   |
| UG-04      | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #324                                                          | 子プロセスは spawn の直後に Job へ登録する（孤児化の窓を最小にする）               |
| UG-05      | 共通基盤     | 共通基盤: ナビゲーション・ルーティング                                 | **引き取り済み** | PR #279                                                          | リリースビルドは起動ルートとテーマを環境変数から取らない                           |
...
| UG-09      | 画面         | 出力                                                                   | **引き取り済み** | PR #323                                                          | 一括出力は既存ファイルを絶対に上書きしない（`_2`, `_3` へ逃がす）                  |
...
| UG-13      | 共通基盤     | 共通基盤: 共通UIコンポーネント（関連: 取込、設定）                     | **引き取り済み** | PR #324                                                          | 長いファイル名がファイル選択ボタンを画面外へ押し出さない                           |
| UG-14      | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #305                                                          | 失敗画面が指すログの場所が、サイドカーが実際に書く場所と一致している               |
| UG-15      | 共通基盤     | 共通基盤: サイドカーライフサイクル・起動ゲート                         | **引き取り済み** | PR #305                                                          | 待たせるときは、待つ理由（初回マイグレーションと Defender スキャン）を出す         |

---

## 18. 共通基盤 52 件の分類とその後の引き取り（Issue #253）
...
Issue #253 の時点で §17 が特定した「共通基盤の未引き取り 52 件」について、いきなりテストを実装せず、実コード・設計根拠に基づく仕分けを行い、以下の 3 つの区分に分類した（分類の件数は当時の実態を保つ）。

| 分類                                  | 意味                                                            |   件数 | 扱い                                                                     |
| ------------------------------------- | --------------------------------------------------------------- | -----: | ------------------------------------------------------------------------ |
| **A: 構造的に起こりえない**           | Electron / 新スタックのアーキテクチャ上、事象自体が発生し得ない |  **7** | 理由と根拠コードを示して記録し、クローズ（テスト不要）                   |
| **B: 既に別のテストが覆っている**     | 先行 PR や画面移植 PR に含まれるテストが既に保証している        | **10** | 覆っているテストファイル名と行番号を特定して記録                         |
| **C: 本当に書く必要がある（要実装）** | 新スタックでも必要な保証であり、まだテストが存在しない          | **35** | 何が必要かを明確化し、後続の独立 Issue に委ねる（本 Issue では書かない） |
| **合計**                              |                                                                 | **52** | **未分類 0 件（7 + 10 + 35 = 52、完全一致）**                            |

> 補足: 本表が分類した 52 件は、分類 C の API クライアント層 6 件（`INV-200`〜`INV-205`）が PR #280、デザイントークン 7 件（`INV-080`〜`INV-086`）が PR #282、アーキテクチャ 2 件（`INV-001`・`INV-002`）が Issue #266、サイドカー起動ゲート 3 件（`INV-033`・`UG-14`・`UG-15`）が PR #305、共通 UI 8 件（`INV-096`・`INV-097`・`INV-104`〜`INV-106`・`INV-108`・`INV-109`・`INV-168`）が PR #308、ルーティング 4 件（`INV-012`・`INV-013`・`INV-016`・`INV-017`）が Issue #268、ドメイン 2 件（`INV-152`・`INV-153`）が Issue #267 で引き取られ、最後に残った分類 A 7 件・分類 B 7 件を PR #324（Issue #322）が実行可能な走査ガード／明示テストとして引き取った。これで 52 件すべてが引き取り済みとなり、§17 の集計は引き取り済み 185・未引き取り 0 になっている。
...
| **INV-080**    | トークン       | **C** | `desktop/test/design-tokens-lint.test.ts`                                                                                                           | `desktop/src/renderer/features/` 配下のスタイル・色リテラル直書き禁止静的検査（PR #282 で引き取り完了）                                                                                       |
| **INV-081**    | トークン       | **C** | `desktop/test/renderer/design-tokens-screens.test.tsx`                                                                                              | 主要画面ルート要素の light/dark テーマ surface ramp 背景色適用検証（PR #282 で引き取り完了）                                                                                                  |
| **INV-082**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | Noto Sans JP フォントのグリフ数（≥17000）および可変軸検証（PR #282 で引き取り完了）                                                                                                           |
| **INV-083**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | 数字表示スタイルの等幅（`tabular-nums`）検証（PR #282 で引き取り完了）                                                                                                                        |
| **INV-084**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | `fontWeight` の指定による可変フォント軸駆動検証（PR #282 で引き取り完了）                                                                                                                     |
| **INV-085**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | 全テキストロールの font-family および line-height 網羅検証（PR #282 で引き取り完了）                                                                                                          |
| **INV-086**    | トークン       | **C** | `desktop/test/typography.test.ts`                                                                                                                   | 認識文字ロールのフォントサイズおよび字間設定検証（PR #282 で引き取り完了）                                                                                                                    |
...
| **INV-200**    | API            | **C** | `desktop/test/sidecar-fetch.test.ts` (PR #280)                                                                                                      | `SidecarClient` に不正なトークンが設定されていても `/healthz` 呼び出しが成功（200）することを API クライアント層で検証するテストが必要。PR #280 で引き取り済み。                              |
| **INV-201**    | API            | **C** | `desktop/test/sidecar-fetch.test.ts` (PR #280)                                                                                                      | `SidecarClient` を通じた正規トークンによる保護 API 呼び出しが成功することを検証するテストが必要。PR #280 で引き取り済み。                                                                     |
| **INV-202**    | API            | **C** | `desktop/test/sidecar-fetch.test.ts` (PR #280)                                                                                                      | `SidecarClient` に不正なトークンを設定した際に保護 API 呼び出しが 401 Unauthorized となることを検証するテストが必要。PR #280 で引き取り済み。                                                 |
| **INV-203**    | API            | **C** | `desktop/test/sidecar-fetch.test.ts`, `desktop/test/sidecar-connection.test.ts` (PR #280)                                                           | `createSidecarClient` において非 loopback アドレスを拒否し、エラーメッセージにトークンや URL を含めない実装とテストが必要。PR #280 で引き取り済み。                                           |
| **INV-204**    | API            | **C** | `desktop/test/sidecar-fetch.test.ts` (PR #280)                                                                                                      | 通信エラー発生時に内部 URL や接続詳細情報を漏洩させないエラーサニタイズ処理とそのテストが必要。PR #280 で引き取り済み。                                                                       |
| **INV-205**    | API            | **C** | `desktop/test/export-conflict.test.ts` (PR #280)                                                                                                    | 409 Conflict 応答のレスポンスボディからサイドカーの拒否コード（`conflictCode`）および設問 ID リストが抽出・保持されることを検証する単体テストが必要。PR #280 で引き取り済み。                 |
...
| **UG-05**      | ルーティング   | **B** | `desktop/src/main/e2e-env.ts`, `desktop/test/e2e-env.test.ts`, `desktop/src/main/main.ts:125-189`                                                   | `readE2eEnv` が `electron.app.isPackaged` を内部参照し、呼び出し側が `false` を渡してゲートを開けられないこと。パッケージ版では E2E 環境変数を無視する。                                      |
...
10. **`UG-05`: リリースビルドは E2E 環境変数を読まない**

- **覆っているテスト**: `desktop/test/e2e-env.test.ts`
- **内容**: `readE2eEnv` が `electron.app.isPackaged === true` のとき `AUTO_SCORING_E2E_FOLDER` / `_PDF` / `_APP_DATA` を無視すること、および `app.isPackaged` を呼び出し側から渡せない実装であることを固定している。`main.ts` のファイル選択はこのヘルパ経由のみ。app-data は `sidecar-app-data.ts` が担当する。

---

### 18.4 分類 C: 本当に書く必要がある（35 件）

これらは新スタックへの移行後も維持すべき約束であり、Issue #253 の時点では既存のテストで覆われていなかった。その後、各領域の後続 Issue（#266・#267・#268・#271・#272・#273・#275・#320・#321・#322 ほか）で順次テストが整備され、本節の 35 件はすべて引き取り済みとなった（個別の引き取り元は §17.4 を参照）。

#### 1. アーキテクチャと依存方向（2 件）

> この 2 件は Issue #266 で引き取り済み（検査範囲は `desktop/test/architecture.test.ts`、引き取り状態は §17 を参照）。
...
#### 5. デザイントークン・アクセシビリティ（7 件中 7 件引き取り完了 ★ PR #282）

- **`INV-080` (スタイルリテラル禁止)**: `desktop/test/design-tokens-lint.test.ts` にて `features/` 配下のスタイル・色リテラル直書きを禁止する静的検査テストを実装（PR #282 で引き取り完了）。
- **`INV-081` (主要画面の surface ramp 背景色)**: `desktop/test/renderer/design-tokens-screens.test.tsx` にて各画面ルート要素の背景色がトークン由来であることを検証（PR #282 で引き取り完了）。
- **`INV-082` (Noto Sans JP variable グリフ数)**: `desktop/test/typography.test.ts` にて配布フォントのグリフ数（≥17000）および可変軸（`fvar`）を検証（PR #282 で引き取り完了）。
- **`INV-083` (数字の等幅設定)**: `desktop/test/typography.test.ts` にて点数等の数字表示スタイルで `tabular-nums` が指定されていることを検証（PR #282 で引き取り完了）。
- **`INV-084` (fontWeight の可変軸駆動)**: `desktop/test/typography.test.ts` にて可変フォントのウェイト軸駆動を検証（PR #282 で引き取り完了）。
- **`INV-085` (全 text role の bundled family + height)**: `desktop/test/typography.test.ts` にて全テキストロールの font-family / line-height 設定を検証（PR #282 で引き取り完了）。
- **`INV-086` (認識文字 role の寸法・字間)**: `desktop/test/typography.test.ts` にて認識文字ロールのフォントサイズ（> 16px）および字間（letter-spacing > 0）を検証（PR #282 で引き取り完了）。

#### 6. 操作要件・無効理由エンジン残余（2 件）

> この 2 件は Issue #271 で引き取り済み（検査範囲は §7.1、引き取り状態は §17 を参照）。
...
```text
共通基盤 52 件の分類結果
├─ 分類 A（構造的に起こりえない）:  7 件 (13.5%) → 追加テスト不要でクローズ
├─ 分類 B（既に別のテストが覆う）: 10 件 (19.2%) → 既存テストで固定済み（引き取り完了）
└─ 分類 C（本当に書く必要がある）: 35 件 (67.3%) → 後続 Issue でテスト整備済み（全件引き取り完了）
```

- **分母の圧縮効果**:
  「共通基盤の未引き取り 52 件」として一括りにされていた課題のうち、**17 件（A の 7 件 + B の 10 件、全体の 32.7%）** は新規にテストを書く必要がないことが判明した。
- **残余テストの実態**:
