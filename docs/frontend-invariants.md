...
を満たすため、`origin/main` にマージ済みの移行 PR 9 本に、本 Issue #273（PR #280）を加えた 10 本の引き取り実績を GitHub App API 経由で取得し、§16 の割当表 185 件（`INV-*` 170 件 + `UG-*` 15 件）と照合した。
...
| **合計** | (10 PR)                                |          **118** | (ユニーク 118 件)                                                                                                     | 重複引き取りなし                                                                                                  |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み 121 件（112 + 3 + 6）に含める。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                 |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **122** |  65.9% | PR #225, #235, #237, #238, #240, #243, #244, #279, #280 および Issue #267 により新スタックで固定済み |
| **未引き取り**   |  **63** |  34.1% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                           |
...
### 17.3 未引き取り 63 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 63 件」の間には **43 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
└─ 共通基盤側の残余: 42 件（PR #279 で UG-05 引き取り後）
...
2. **共通基盤側の未引き取り（42 件、PR #279 反映後）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #280（6 件: `INV-200`〜`205`）、PR #279（1 件: `UG-05`）が引き取った 32 件を除く **42 件**が依然として未引き取りである。
...
**結論:** cut-over 条件である「185 件の不変条件を覆うこと」を満たすには、残る未移植 3 画面（答案確定・設定・出力）とテスト設定プロファイルの移植だけでは不足であり、**共通基盤の未引き取り 42 件を固定する独立した Issue / テスト整備が必要**である。

### 17.4 不変条件 185 件の完全照合一覧表

§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #280）および Issue #267 の引き取り実績と突き合わせた完全照合表を以下に示す。

- **引き取り済み:** 計 **122 件**（新スタックのテストで固定済み）
- **未引き取り:** 計 **63 件**（未移植画面 21 件 + 未引き取り共通基盤 42 件）
- **二重計上:** **0 件**（主担当としての重複引き取りなし）
- **合計件数:** **185 件（122 + 63 = 185、完全一致）**
...
| **UG-05**      | ルーティング   | **B** | `desktop/src/main/e2e-env.ts`, `desktop/test/e2e-env.test.ts:5-24`, `desktop/src/main/main.ts:125-189`                                              | E2E 用環境変数は `app.isPackaged === false` のときだけ読み、パッケージ版では無視されることを `readE2eEnv` の単体テストで固定している。                                                        |
...
- **覆っているテスト**: `desktop/test/e2e-env.test.ts:5-24`
- **内容**: `readE2eEnv` が `app.isPackaged === true` のとき `AUTO_SCORING_E2E_FOLDER` / `_PDF` / `_APP_DATA` を無視することを固定している。`main.ts` のファイル選択・app-data 上書きはこのヘルパ経由のみ。
