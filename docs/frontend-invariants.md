...
| **合計** | (14 PR)                                          |          **130** | (ユニーク 130 件)                                                                                                     | 重複引き取りなし                                                                                                                        |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）、Issue #268 が引き取った 4 件（`INV-012`, `INV-013`, `INV-016`, `INV-017`）、および Issue #192 が引き取った 2 件（`INV-015`, `INV-019`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み **139 件（PR 単位 130 + Issue 名義 3 + 4 + 2）** に含める。この等式（`§17.2 の引き取り済み = 本表の合計 + Issue 名義の引き取り`）は `desktop/test/frontend-invariants-tally.test.ts` が検査する（§17.0 参照）。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                                               |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **139** |  75.1% | PR #225, #235, #237, #238, #240, #243, #244, #279, #280, #282, #283, #291 および Issue #267, #268, #192 により新スタックで固定済み |
| **未引き取り**   |  **46** |  24.9% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                                                         |
...
### 17.3 未引き取り 46 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 46 件」の間には **26 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
└─ 共通基盤側の残余: 25 件（PR #279・#282・本 Issue #266・Issue #192 反映後）
...
2. **共通基盤側の未引き取り（25 件、PR #279・#282・本 Issue #266・Issue #192 反映後）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #280（6 件: `INV-200`〜`205`）、PR #283（2 件: `INV-004`, `INV-101`）、Issue #268（4 件: `INV-012`, `INV-013`, `INV-016`, `INV-017`）、PR #279（1 件: `UG-05`）、PR #282（7 件: `INV-080`〜`INV-086`）、本 Issue #266（2 件: `INV-001`, `INV-002`）、Issue #192（2 件: `INV-015`, `INV-019`）が引き取った 49 件を除く **25 件**が依然として未引き取りである。
...
**結論:** cut-over 条件である「185 件の不変条件を覆うこと」を満たすには、残る未移植 3 画面（答案確定・設定・出力）とテスト設定プロファイルの移植だけでは不足であり、**共通基盤の未引き取り 25 件を固定する独立した Issue / テスト整備が必要**である。
...
> 補足（Issue #273 / PR #280）: 本表が分類した 52 件のうち、分類 C の API クライアント層 6 件（`INV-200`〜`INV-205`）は PR #280 で引き取り済みとなった。§17 の集計は引き取り済み 137・未引き取り 48 に更新されている。
