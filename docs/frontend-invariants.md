...
を満たすため、`origin/main` にマージ済みの移行 PR 9 本に、Issue #273（PR #280）と本 Issue #271 の PR を加えた 11 本の引き取り実績を GitHub App API 経由で取得し、§16 の割当表 185 件（`INV-*` 170 件 + `UG-*` 15 件）と照合した。
...
| **合計**  | (11 PR)                                |          **120** | (ユニーク 120 件)                                                                                                     | 重複引き取りなし                                                                                                  |

Issue #267 が引き取った 3 件（`INV-006`, `INV-152`, `INV-153`）、Issue #268 が引き取った 4 件（`INV-012`, `INV-013`, `INV-016`, `INV-017`）、および Issue #192 が引き取った 2 件（`INV-015`, `INV-019`）は PR 単位ではないため本表には含めず、§17.2 の引き取り済み 129 件（120 + 3 + 4 + 2）に含める。
...
| 状態区分         |    件数 |   割合 | 概要                                                                                                             |
| ---------------- | ------: | -----: | ---------------------------------------------------------------------------------------------------------------- |
| **引き取り済み** | **129** |  69.7% | PR #225, #235, #237, #238, #240, #243, #244, #280, #283 および Issue #267, #268, #192 により新スタックで固定済み |
| **未引き取り**   |  **56** |  30.3% | 未移植画面（答案確定・設定・出力・テスト設定残余）および未引き取り共通基盤                                       |
...
### 17.3 未引き取り 56 件の内訳と未移植画面との差分分析

「未移植の 3 画面（答案確定 7 件 + 設定 1 件 + 出力 12 件 = 計 20 件）」と「未引き取り 56 件」の間には **36 件の差**が存在する。どこで何が落ちているかを精査した結果、以下の構造が判明した。
...
└─ 共通基盤側の残余: 35 件
    ├─ ナビゲーション・ルーティング: 4 件（INV-010, INV-011, INV-020, UG-05）
...
2. **共通基盤側の未引き取り（35 件）:**
   - 割当表 §16.4 の共通基盤 74 件のうち、PR #237（21 件）、PR #235（1 件: `INV-201-05`）、Issue #267（3 件: `INV-006`, `INV-152`, `INV-153`）、PR #280（6 件: `INV-200`〜`205`）、PR #283（2 件: `INV-004`, `INV-101`）、Issue #268（4 件: `INV-012`, `INV-013`, `INV-016`, `INV-017`）、Issue #192（2 件: `INV-015`, `INV-019`）が引き取った 39 件を除く **35 件**が依然として未引き取りである。
...
**結論:** cut-over 条件である「185 件の不変条件を覆うこと」を満たすには、残る未移植 3 画面（答案確定・設定・出力）とテスト設定プロファイルの移植だけでは不足であり、**共通基盤の未引き取り 35 件を固定する独立した Issue / テスト整備が必要**である。

### 17.4 不変条件 185 件の完全照合一覧表

§16.5 の割当表 185 件（`INV-201-01`〜`INV-201-10`、`INV-001`〜`INV-205`、`UG-01`〜`UG-15`）すべてについて、移行 PR（#220, #225, #231, #235, #237, #238, #240, #243, #244, #280, #283）および Issue #267, #268, #192 の引き取り実績と突き合わせた完全照合表を以下に示す。

- **引き取り済み:** 計 **129 件**（新スタックのテストで固定済み）
- **未引き取り:** 計 **56 件**（未移植画面 21 件 + 未引き取り共通基盤 35 件）
- **二重計上:** **0 件**（主担当としての重複引き取りなし）
- **合計件数:** **185 件（129 + 56 = 185、完全一致）**
