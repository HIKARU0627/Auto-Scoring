...
- 起動ごとに `secrets.token_urlsafe(32)`（256bit）でトークンを生成する。
  環境変数 `AUTO_SCORING_SIDECAR_TOKEN` があればそれを使う（テスト・親プロセス指定用）。
...
- `--handshake-file <path>`: 指定ファイルへ `{"host","port","token"}` の JSON を 1 行書く。**推奨**。
- 省略時: 標準出力の 1 行目に同じ JSON を書く（開発時のフォールバック）。

トークンはハンドシェイク経路にのみ書き出す。アプリケーションログには
`_RedactingFilter` が `***` へ置換して出さない（`install_log_redaction`）。
