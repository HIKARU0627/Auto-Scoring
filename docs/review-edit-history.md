...
## 14. 答案 1 枚の確定は、設問ごとの確定を N 回呼ぶ（Issue #145）

確定の単位が設問から**答案 1 枚**へ移った（`docs/review-queue.md` §11）。
**API は 1 つも足していない。**

### 14.1 schema を変えなかった

`POST .../questions/{question_id}/reviews/approve` を、その答案の未確定の設問ぶん
**順に N 回呼ぶ**。答案単位の確定エンドポイントは作っていない。

理由は 2 つある。1 つは、この波で Issue #102 の担当が OpenAPI の生成物を持って
いて、schema を触れば衝突すること。もう 1 つは、**#22 が `reviews` に積み上げた
ものが設問粒度だから**である（§1・§2）。`version` は `(submission_id, question_id)`
ごとの連番で、`UNIQUE(submission_id, question_id, version)` がそれを守っている。
答案単位の確定を足すなら、この楽観ロックを答案の粒度で組み直す話になり、
それは「1 画面で確定する」ためだけに払う値段としては高い。

### 14.2 だから部分失敗が起こる。**隠さない**

N 回のうち 3 回目で失敗すれば、2 問は確定済みで答案は中途半端な状態で残る。
「確定できませんでした」とだけ言う画面は、**すでに確定した 2 問の存在を人から
隠す**。「1 回で確定」を、失敗を隠す言い訳にしてはならない。

`core/submission_confirmation.dart` の `runSubmissionConfirmation` が返すのは
成否ではなく**どこまで進んだか**である。

| 返すもの                   | 何のため                                                       |
| -------------------------- | -------------------------------------------------------------- |
| `confirmed`                | この操作で確定できた設問番号。**失敗した回でも空とは限らない** |
| `failedNumber` / `message` | どの設問で、サイドカーが何と言ったか                           |
| `remaining`                | 確定できずに残った設問番号（失敗したものを含む）               |

画面はこれを 1 文にして出す —
「問1・問2 を確定しました。問3 で失敗しました: 他の操作と競合しました。
残り2問（問3・問4）はそのまま確定し直せます。」

**最初の失敗で止める。** 失敗の多くは競合（409）で、誰かがこの答案を触っていた
なら後続の `expectedVersion` も同じように古い。押し通しても失敗が並ぶだけで、
どこで何が起きたかは読みにくくなる。

**失敗のあとは必ず全設問を読み直す。** 確定できた設問の履歴は伸びているので、
読み直さないと再実行が古い `expectedVersion` を送り、**直せる失敗が直せない失敗
になる**。読み直したあとは残りだけが確定対象になり、ボタンのラベルも
「2問をまとめて確定」に変わる — 押すのは「もう一度全部」ではなく「残り 2 問」で
あり、ボタンがそう言わなければ、どこまで済んだかを覚えているのは人の側になる。

### 14.3 承認できない設問は、確定そのものを止める

`Review.APPROVED` は `ai_grade_result_id` を必須にしている（§1、`domain/models.py`）。
AI の点数が無い設問は**承認しようがない**ので、その設問を名指しして答案全体の
確定を止め、「点数を入力する」で添削レビュー画面（§3.1、Issue #118）へ送る。

止めずに残りだけ確定させることもできたが、**確定の単位が答案 1 枚である以上、
1 問が欠けたまま「確定した」と言ってはならない**。それは部分失敗を、失敗と呼ばずに
起こすことである。
