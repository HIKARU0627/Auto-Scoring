...
| 再表示          | **しない。** 保存済みかどうかと、どこから読んだかだけを出す                                 |
...
| `AUTO_SCORING_AI_GRADING_TRANSPORT` | **環境変数**       | 開発機の Vertex 優先（Issue #81 判断 B）を変えない |
| モデル・prompt version              | **環境変数**       | 環境が何も言わないときだけ、配布物向けの既定を使う |

画面はこの 2 つを両方とも「どこから読んだか」つきで表示する。開発機は
...
OpenRouter の `GET /api/v1/key`（推論を走らせないので無課金）を 1 回叩き、
成功・認証失敗・ネットワーク不通・provider エラー・残高切れを別々に表示する。
