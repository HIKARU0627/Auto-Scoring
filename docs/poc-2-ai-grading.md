...
他設問・生徒識別情報は含めない。**この制約は「採点」経路のものとして現行仕様でも有効**
— 2026-09-09 の目的別整理でも採点は回答欄の切り出しのみである。
ページ全体を送るのは回答欄の**検出**経路（Issue #105）だけで、本 PoC の対象外）
と OCR テキスト（`ocr_text`）の**両方**を
...
- 採点基準）だけ**に限定する（決定書 §2 (2)）。**この制約は「採点」経路のものとして
  現行仕様でも有効である**（2026-09-09 の目的別整理でも採点は回答欄の切り出しのみ）。
  ページ全体を送るのは回答欄の**検出**経路（Issue #105）だけで、本 PoC の対象外。
  可能なら各サービスの
...
2. **`responseJsonSchema` は `maxLength` を強制しない**。初回の probe で
   ...
   スキーマが示す長さ制限を守るよう明示する 1 文を追加したところ、同じセルで
   再現しなくなった。ベンダー名も具体的な数値も含まない指示であり、特定 provider
   への最適化ではない。**書式の不一致が採点品質の差として集計されるのを防ぐための
   修正**である。

   **2026-09-09 追記（Issue #121）**: この 1 文だけでは足りなかった。実データでは
   同じ超過が再現し、AI 採点の恒久失敗 6 件のうち 5 件を占めた。上限超過は
   **応答を捨てるのではなくコメントを切り詰める**ように変え、プロンプトの文面も
   「超過は全体が無効になる」から「超過分は切り捨てられる」に直した
   （`docs/ai-grading-pipeline.md`「上限を超えた注釈コメントは切り詰める」）。
   ここに残っている「プロンプトを足したら再現しなくなった」という観測は
   **合成フィクスチャ 6 セルでの話**であり、実データでは成り立たなかった。
