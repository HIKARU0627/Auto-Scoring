...
> 点数についてはこの導出を撤回した（§2.2.3、Issue #159）。関数は
> `derive_comment_area` になり、導出するのはコメント帯だけである。
> コメント帯が同じように重なっている件は §2.2.3 の「残っている穴」。
...
`derive_mark_areas` は `derive_comment_area` になり、`score_area` は `SCORE` region が
無ければ `None` のまま。点数の位置は `fallback_score_areas` が出力時に解決する。
...
##### 残っている穴: 注釈コメントは筆跡の上に印字され続ける

**#159 で直したのは点数だけである。** `comment_area` は同じ「回答欄の真下の帯」から
出ており、同じ計測で同じように壊れている（最悪 19.1%、他に 7.1%、4.4%）。
**この変更を入れても、注釈のコメント文は生徒の筆跡の上に印字される。**

同じ PR に入れなかったのは、直し方が同種ではないからである。点数は3〜5文字なので
幅3%の帯に収まるが、注釈コメントは散文で、同じ帯に入れれば省略記号で切られ続ける
（§「帯に収まらないときは、やはり断る」）。退避先の設計をやり直す話になり、
#159 の目的から外れる。別Issueとして扱う。

なお #141 は「位置が特定できない注釈のコメント文は `comment_area` へ」という設計だった。
その `comment_area` 自体が筆跡に重なっているなら、**#141 が救ったつもりのコメントも
答案を汚している。**
