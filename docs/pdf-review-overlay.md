...
- 記録するのは真偽値ではなく到達時の `maxScrollExtent`。読み返しのために
  上へ戻しても「見た」は消えない（位置は変わるが extent は変わらない）一方、
  あとから材料が増えた場合（pollingがgradeを運んできた、Undoで行が戻った）は
  extent が伸びるので、短い画面で得た既読は自動的に無効になる。
- 未読のあいだは 承認して次へ が無効（`_canApprove`）。無効な理由はボタンの
  tooltipと、判断材料の先頭に出る誘導行（アイコン＋文＋「続きを表示」ボタン、
  色だけに依存しない）で示す。
...
誘導行は**アクションバーの上ではなく、判断材料スクロール領域の先頭**に置いて
いる。アクションバーの上に置くと、「まだ続きがある」と言うための行が、その
続きを表示するための高さを自分で削る。**あと少しで収まる画面では、誘導行を
出したこと自体が収まらない原因になり、二度と消えない。** 自分と矛盾する
レイアウトになるので、材料の中に置いた。同じ理由で、パネルに実際に下端外の
内容があること（`_materialExtent > 0`）を確認してから出す。

#### 残る限界

材料が既読になったあと、polling が**内容を書き換えずに**行を増やした場合の
検知は `maxScrollExtent` の増減に依存する。高さが変わらない差し替え
（同じ行数で文言だけ変わる）は既読のままになる。ここを厳密にやるには
