...
- 判断材料の各行が画面に入ったことを記録する（`_seenMaterialRowIds`）。
  **全部が一度に画面に収まっている場合は、最初のレイアウト直後にその場で
  ...
  - 初期表示だけを見るテストでは足りない（レビュー1回目）。**状態が変わる
    経路**を通す必要がある（後述）。
- 記録するのは真偽値でも寸法でもなく、**行ごとに「その行の上端と下端が画面に
  入ったか」**（`_seenMaterialRowIds`）。行は OCR認識・採点AIの認識・人による
  修正・AIの採点・人による確定・採点基準・設問コメントで、それぞれのブロックの
  両端に高さ0のマーカーを置いて位置を測る。

  既読は「表示中の全行が、両端とも画面に入ったことがある」。読み返しのために
  上へ戻しても消えず、あとから材料が変わった場合（pollingがgradeを運んできた、
  Undoで行が戻った）は**その行が新しい行になる**ので、その行だけが未読に戻る。
  ...
  **下端だけでは足りない**（レビュー4回目 P2-1）。末尾で止まっている読み手の
  上で行が長いものに差し替わると、その行の**下端だけが画面に入ってくる**
  （実測で上端 -113 / 下端 56、ビューポート高 214）。下端が見えたことだけを
  根拠にすると、一言も表示されていない行が既読になる。上端と下端の両方を
  要求すれば、その行は上へ戻るまで未読のままである。

- 未読のあいだは 承認して次へ が無効（`_canApprove`）。無効な理由はボタンの
  tooltipと、パネルの下端に浮かぶ誘導行（アイコン＋文＋「続きを表示」ボタン、
  色だけに依存しない）で示す。
- **「続きを表示」は1画面ぶんずつ送る。** 末尾へ飛ぶと、あいだの行は一度も
  表示されないまま通り過ぎる ——「続きを表示」と書いておいて表示しないことに
  なる。少し重ねて送るので、行が2画面の隙間に落ちることもない。
...
ここは**4回**間違えた。4回とも、パネル全体について1つのスカラを見ていた。
...
「何を通過したか」を答えられない。だから**行ごとに、その行の両端が画面に
入ったか**を記録する形にした。スクロールの実測値は「**いま**その行の端が画面に
出ているか」という一瞬の判定にだけ使い、記録には残さない。

- レイアウトが変わっても行のidは変わらないので、1〜2回目の誤爆は構造的に
  起きない
- 画面外に現れた新しい行は端が画面に入っていないので、3回目の誤爆も構造的に
  起きない
...
対になるのが `材料そのものが増えたときは未読に戻る` である。これが無いと、
「絶対に閉じないゲート」が上のテストを全部通ってしまう。あとから新しい認識
結果の行が届いたときに未読へ戻ることを確認する。

- 未読のあいだは 承認して次へ が無効（`_canApprove`）。無効な理由はボタンの
  tooltipと、パネルの下端に浮かぶ誘導行（アイコン＋文＋「続きを表示」ボタン、
  色だけに依存しない）で示す。
- **Enter は「無反応」にしない。** 未読のあいだの Enter は承認せず、
  残りの判断材料を表示したうえで理由をSnackBarで告げる
  （`_approveAndNext` 冒頭）。押しても何も起きない画面は、理由が
  伝わらないという意味では画面外の判断材料と同じ問題である。
  加えて ↑/↓ はこの画面では設問移動に割り当たっているので、
  **キーボードだけの利用者が残りを表示する経路はここしか無い**
  （「続きを表示」ボタン側でも Enter を `ActivateIntent` に再束縛して
  あるので、Tabで到達すれば押せる）。
- **既に確定済みの設問では素通しする。** その場合の 承認して次へ は確定では
  なく単なる移動なので、止める理由がない。
- **却下・再判定・修正・元に戻すはゲートしない。** 却下と再判定は「AIの結果を
