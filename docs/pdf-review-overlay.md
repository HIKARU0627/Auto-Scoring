...
- Inspectorは440px幅（従来360px）。設問見出しと §2.13 の設問状態チップを
  ...
  （縦に積むか横に並べるかを変えただけ）。
- 狭幅でInspectorが取る高さには**下限**を置いた
  （`AppLayout.reviewNoteBandHeight`）。固定した修正コメント欄は高さの決まった
  子なので、割り当てがそれを下回るとパネルが縮むのではなく**桁あふれする**。
  素の `Expanded(flex: 2)` には下限が無く、Issue #80 が「AI採点を開始」の通知を
  このペインの上に足したあと、残りの2/5がこの帯を下回った（700x420で11px
  あふれた）。同じ理由で、その通知自体は高さ配分の外に置いてある — ボタンを
  持つ行動喚起であり、縮められる側になってはいけない。
...
- Inspectorのスクロール末尾に一度到達したことを記録する
  （`_seenMaterialKeys`）。**スクロールの余地が無い＝全部見えている場合は
  最初のレイアウト直後にその場で条件を満たす。** 標準幅の通常ケースで
  ゲートは一切発動しない。ここは
  ...
  - 初期表示だけを見るテストでは足りない（レビュー1回目）。**状態が変わる
    経路**を通す必要がある（後述）。
- 記録するのは真偽値でも寸法でもなく、**そのとき表示していた行の組**
  （`_materialIdentity`）。OCR認識・採点AIの認識・人による修正・AIの採点・
  表示中のgrade・設問コメントの各行のidを並べた文字列である。読み返しの
  ために上へ戻しても「見た」は消えず、あとから材料が変わった場合
  （pollingがgradeを運んできた、Undoで行が戻った）は行が変わるので既読は
  自動的に無効になる。

  idで足りるのは**履歴がappend-onlyだから**である。`Recognition` /
  `GradeResult` / `Annotation` は書いたら二度と編集されず、人の修正も再判定も
  Undoも**別の行**として増える（[review-edit-history.md](./review-edit-history.md)）。
  同じidを出している2つの画面は、どんな幅で描かれていようと同じ文言を出して
  いる。
...
| 問い                       | 何で答えるか                  | レイアウト依存 |
| -------------------------- | ----------------------------- | -------------- |
| 材料が変わったか           | `_materialIdentity`（行のid） | しない         |
...
2 と 3 が重なって鳴るのは無害である。`_recordMaterialRead` は本当に変わった
...
#### なぜ「描画されたもの」で判定しないのか（レビュー1回目・2回目 P2）

ここは2度間違えた。**どちらも「レイアウトが変わった」と「材料が増えた」を
区別できなかった。**

| 記録していたもの                               | 誤爆した経路                                                                                           |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `maxScrollExtent`                              | 窓を縮める／修正コメントを3行打つ。どちらもビューポートが縮むだけで extent が伸びる                    |
| 判断材料の高さ（`maxScrollExtent + viewport`） | 700x720 で読み終えてから 1280x720 へ広げる。Inspectorが440px幅の列になり、折り返しが増えて高さが伸びる |

2つ目は1つ目の修正として入れたもので、**窓の高さへの依存を捨てて幅への依存を
拾っただけ**だった。テキストが折り返す以上、高さは幅の関数である。

**描いた結果はどれも「描き方」についての事実であって、「材料」についての事実
ではない。** レイアウトの影響から逃れるには、描かれたものを測るのをやめる
しかない。だから記録は行の組にした。スクロールの実測値は
「**いま**末尾が画面に出ているか」という一瞬の判定にだけ使い、記録には残さない。

同じ形の問題は同じ日に3件出ている（#73 のスクリーンショット鮮度判定、
