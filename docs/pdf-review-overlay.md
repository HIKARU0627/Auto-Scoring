...
**この判定は答案確定画面（Issue #145）でも同じものを使う。** まとめて確定する
画面は、ここで消したはずの「判断材料を見ないまま承認ボタンだけ押せる」を
**設問の数だけ作り直しうる**。行の被覆を数える算術（`core/material_read_ranges.dart`）
を両画面が共有しているので、片方だけが緩むことはない。決定は
`docs/review-queue.md` §11.3。
...
### 2.16 Electron 版: 採点完了への追従と「続きを表示」の方向（Issue #319）

Electron 版の添削レビュー画面は、`reload()` を**マウント時に一度**、`loadJobs()` を
**何らかの操作の後**にしか呼んでいなかった。ホームの「レビューを続ける」は採点が
終わる前にこの画面へ入るため、入った時点でジョブが `running` だと
`AI処理中` のまま止まり、承認ボタンは永久に無効のままだった。実機では採点が
投入の 32 秒後に成功していた一方、画面は最後に jobs を読んだ 0.6 秒後から
10 分間リクエストを出していない（＝競合状態で、完了後に入れば正常）。
表示済みのジョブを定期取得して追従するようにした。

- **間隔 2 秒**（`JOB_POLL_INTERVAL_MS`）。実測のジョブは 32 秒で終わっており、
  2 秒なら「操作しなくても勝手に承認できるようになる」と感じられる。サイドカーは
  単一ワーカーなので、これ以上短くすると無駄に叩く。
- **打ち切り 150 回**（`JOB_POLL_MAX_ATTEMPTS`、2 秒 × 150 = 5 分）。実測の
  ~32 秒に対して約 10 倍の余裕。5 分たっても終わらない採点は自動追従を諦め、
  `grading-status-stale` の通知を出して「再読み込み」を促す。**黙って止まらない。**
- **走っているジョブが無くなったら即座に止める。** 追従するのは
  `queued` / `running` / `blocked` のときだけで、`succeeded` / `failed` /
  `cancelled` しか無ければタイマーを張らない。採点済みの画面を叩き続けない。
- **完了した設問の判断材料を取り直す。** ジョブが終端に変わったら、その設問の
  grades / recognitions / annotations / 切り出し画像を読み直す。ジョブ一覧だけ
  更新しても点数が古いままでは追従にならない。
- **手動の再読み込み**（`review-refresh-button`）を置く。答案確定画面の
  `confirm-refresh-button` と同じ役割で、自動追従が壊れても利用者が自力で抜けられる。
  押すと追従の打ち切り状態もリセットする。
- **文言は `core/action-requirements.ts` が単一ソース**（INV-004）。
  `grading-in-progress`（なぜ承認できないか）と `grading-status-stale`
  （更新が取れていないこと）を追加した。

**「続きを表示」の方向も直した。** `unreadIsAbove` は計算されていたのに
`revealRest()` が使っておらず、常に下へスクロールしていた。下端で止まっている
あいだに画面より上の行が未読として残ると、押しても何も動かずゲートが開かない
（Flutter 版のレビュー5回目 P2 と同じ壊れ方）。`core/material-read-ranges.ts` に
`materialRowGapStart` を足し、**最初の未読部分がビューポートより上か**で上下を
決める。行がビューポートより高い場合、行の上端が画面外にあっても未読は下に
あるため、`rowTop < containerTop` だけでは向きを誤る。

### 2.17 Electron 版: 「続きを表示」は未読行そのものへ送る（Issue #328）

#319 の方向修正後も、実機では **承認が一度もできない** 再現が残った。容器は
`scrollHeight 2224 / clientHeight 608` でスクロール可能なのに、「続きを表示」は
`scrollTop 0 → 547`（`clientHeight * 0.9`）まで一度進んだきり、以後 5 回押しても
1px も動かず、材料行 2 件は最後まで表示域に入らなかった（ビューポートより高い
行は 0 件）。547 はまさにページ送り 1 回分である。

原因は **相対のビューポートページ送りが必ず積み上がるとは限らない**こと。
`scrollBy({ top: clientHeight * 0.9, behavior: "smooth" })` は、進行中の
スムーススクロールと測定の再レンダリングが重なると、2 回目以降が同じ位置に
留まることがある（Electron 実測）。ページ送りは「あと何回で届くか」を
呼び出し側が推測する設計で、届かない位置で止まると **押しても無反応のまま
ゲートが閉じ続ける**。

- **未読の先頭そのものを行の高さの割合から求め、その点をビューポート上端へ
  合わせる**（`core/material-read-ranges.ts` の `planMaterialReveal`）。ページ
  単位ではなく未読点を狙うので、押すたびに必ず近づく。向き（#319）も同時に
  満たす。
- **絶対位置（`container.scrollTop`）で移動する。** 相対 `scrollBy` に依存
  しないため、スムーススクロールの取りこぼしが起きない。
- **未読点が既に上端にあるときは 1 ビューポートぶん進める。** 整列が no-op に
  なって止まることを防ぐ。スクロール範囲が 0 でどうしても動かせない場合は
  `moved: false` を返し、成功したように見せかけず「これ以上『続きを表示』では
  進めませんでした。判断材料を手動でスクロールして確認してください。」と出す
  （手動スクロールでもゲートは開く）。
- **ゲートは緩めない。** #261 の「判断材料を読んでから承認する」は意図的な設計で、
  直すのは読ませ方だけである。

回帰テストは本 Issue の実測と同じ形（容器 608px・内容 2224px・行 2 件がどちらも
表示域外）を組み、「続きを表示」を繰り返すと `allRowsCovered` が true になる
