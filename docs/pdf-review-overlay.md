...
- Inspectorのスクロール末尾に一度到達したことを設問ごとに記録する
  （`_materialSeenExtent`）。**スクロールの余地が無い＝全部見えている場合は
  `maxScrollExtent == 0` を最初のレイアウト直後に記録するので、その時点で
  条件を満たす。** 標準幅の通常ケースでゲートは一切発動しない。ここは
  ...
  - 初期表示だけを見るテストでは足りない（レビュー1回目）。**状態が変わる
    経路**を通す必要がある。現在は幅の変化・ウィンドウ高の変化・メモ欄が
    伸びることによるビューポートの変化を受入テストで固定している。
- 記録するのは真偽値ではなく到達時の**判断材料そのものの高さ**
  （`maxScrollExtent + viewportDimension`）。読み返しのために上へ戻しても
  「見た」は消えない（位置は変わるが高さは変わらない）一方、あとから材料が
  増えた場合（pollingがgradeを運んできた、Undoで行が戻った）は高さが伸びるので、
  短い画面で得た既読は自動的に無効になる。
  - **`maxScrollExtent` そのものではない**（レビュー1回目 P2）。あれは
    「材料の高さ − ビューポート」であり、窓の話が混ざっている。ウィンドウを
    縮めても、修正コメントを3行打ってビューポートが縮んでも、`maxScrollExtent`
    は伸びる。**一度画面に出しきったという事実は、後から窓を小さくしても
    取り消されない。** 材料が実際に増えたときだけ未読へ戻すには、
    材料の高さだけを見る必要がある。
...
材料が既読になったあとの変化の検知は、材料の高さの増減に依存する。高さが
変わらない差し替え（同じ行数で文言だけ変わる）は既読のままになる。ここを厳密にやるには
「何を見たか」を内容ハッシュで持つ必要があり、3秒ごとの再取得と噛み合わない。
現状は「高さが伸びたら未読に戻す」で十分と判断した。
