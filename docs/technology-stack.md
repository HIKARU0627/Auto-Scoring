...
- 「PoC 後に判断」と簡易設計書が明記していた項目（OCR サービス、AI モデル）は、本書の初版では
  最終確定せず **PoC の判断材料**と**抽象化の境界**だけを固めた。両者は
...
| 論点                                                        | 現時点の方針                                                                                                                                                                                                                           | 出典                                                                                                                                       |
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 答案データ（個人情報を含む）の外部 AI/OCR クラウド API 送信 | **可**。ただし**目的で範囲が変わる**: **採点＝回答欄の切り出しのみ**（識別情報は渡らない）／**回答欄の検出＝ページ全体**（書かれていれば渡る）。資料（採点基準・添削資料・添削サンプル）は文書全体を送ってよい。**公開は引き続き禁止** | ユーザー確認済み／[Issue #95](https://github.com/HIKARU0627/Auto-Scoring/issues/95) 決定 7、業務ルール §2 (2)（2026-09-09 に目的別へ整理） |
...
| 画面フォント      | **Noto Sans JP を同梱**（可変フォント1ファイル、SIL OFL 1.1）。選定理由・却下候補・ライセンス確認は [design-tokens.md](./design-tokens.md) §2 | OS 同梱フォント参照: 開発機(Linux)と配布先(Windows)で字形が変わり、画面比較が成立しない。`google_fonts`: 初回描画にネットワークを要求する（オフライン前提に反する）                       |
...
| Google 認証          | `google-auth`（**ADC のみ**。ベンダー SDK は入れない）               | Vertex AI/Gemini は API キーが組織ポリシーで禁止（Issue #35）。呼び出し自体は素の HTTPS なので `httpx` で足り、必要なのはトークン取得だけ。google-auth 同梱の transport は `requests`/`urllib3` 用のため、`httpx` 上に薄い transport を書いて 3 つ目の HTTP ライブラリを増やしていない（[`poc-2-ai-grading.md`](./poc-2-ai-grading.md) §7.1） |
...
| 並行数     | 設定値（**既定 4**、`QueueSettings.max_concurrency`）。外部 AI のレート制限に合わせて調整。§33-24 は Issue #81 で確定 |
...
- **具体サービス・モデルは Issue #81 で確定**（§33-2, §33-3。
  [`business-rules-and-evaluation-data.md`](./business-rules-and-evaluation-data.md) §3 A・B）。
  OCR は **Google Document AI**、AI は **優先度つきフォールバック**（Gemini API → Codex App
  Server → OpenRouter → OpenAI API）。以下の候補表は PoC 1 / PoC 2 時点の比較対象であり、
  採用結果を「採用」欄で示す。

#### OCR 候補と採用結果（PoC 1 で比較、Issue #81 で確定）

| 候補                                               | 位置づけ                                                                                                                 |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Google Document AI**                             | **採用（Issue #81）**。手書き対応の OCR プロセッサを持ち、単語単位の Bounding Box が取れる（§12.3 の注釈位置決定に必須） |
| Google Cloud Vision API（DOCUMENT_TEXT_DETECTION） | PoC 1 の第一候補だったが不採用。同じ Google だが API・レスポンス形式は別物なので流用しない                               |
...
#### AIProvider の採用順序（Issue #81 で確定）

**単一モデルの選択ではなく、優先度つきフォールバック**である。上から順に試し、失敗したら
次へ落とす（どの失敗で落とすかと層の置き場所は
[`ai-grading-pipeline.md`](./ai-grading-pipeline.md)「AI モデル: 優先度つきフォールバック」）。

| 優先度 | provider                             | 位置づけ                                                                                          |
| ------ | ------------------------------------ | ------------------------------------------------------------------------------------------------- |
| 1      | Gemini API                           | 大きめのコンテキスト・コスト効率。画像＋ルーブリック同時入力（§8.1）に向く。アダプタ未実装        |
| 2      | Codex App Server                     | Issue #44 で実装済み（`CodexAppServerProvider`）。API キーではなく `codex login` セッションを使う |
| 3      | OpenRouter（オープンウェイトモデル） | Issue #44 で実装済み（`OpenRouterAIProvider`）。1 キーで多ベンダーへルーティング                  |
| 4      | OpenAI API                           | 構造化出力ツールが充実。アダプタ未実装                                                            |
| —      | ローカル LLM                         | 送信不可データ用のフォールバック（現時点はクラウド可のため優先度外）                              |
...
- クラウド AI/OCR 送信は許可されている。**Issue #95 決定 7 で「設問単位の必要最小限」という
  一律の制約は外れ、2026-09-09 に目的別へ整理した**（業務ルール §2 (2)、簡易設計書 §26.1.1）。

  | 目的             | 送るもの             | 生徒識別情報                     |
  | ---------------- | -------------------- | -------------------------------- |
  | **回答欄の検出** | **ページ全体の画像** | **書かれていれば渡る**           |
  | **採点**         | **回答欄の矩形だけ** | ヘッダを含まないので**渡らない** |

  資料（採点基準・添削資料・添削サンプル）は文書全体を送ってよい。

- **「生徒識別情報を一切送らない」とは書かない。維持できない約束だから。**
  採点では渡らないが、**検出ではページ全体を送るので記入されていれば渡る**。
  **手書きの氏名を確実に検出して消す手段は無い。**
  実資料では氏名欄が空欄だったが、**次の資料で埋まっている可能性がある**
  （業務ルール §2 (2)「除去を前提に設計しない」）。
- **「provider へ送ってよい」は「公開してよい」ではない。** commit / PR / Issue / docs /
  CI ログ / スクリーンショット / テスト fixture への持ち出しは引き続き禁止。
...
| 2             | 使用する AI モデル                                                       | **確定（Issue #81）**。優先度つきフォールバック: Gemini API → Codex App Server → OpenRouter → OpenAI API。`AIProvider` で隔離 |
| 3             | 使用する OCR                                                             | **確定（Issue #81）**: Google Document AI。`OCRProvider` で隔離                                                               |
...
| 19            | 低 Confidence の基準値                                                   | **確定（Issue #81）**: 固定値を置かず設定値のまま。既定 0.80 を据え置き運用調整                                               |
| 22            | キーボードショートカット                                                 | §17 の候補で実装開始。最終割り当てはユーザー確認                                                                              |
| 23            | AI 自動コメント生成の許容範囲                                            | 業務ルール。ユーザー判断                                                                                                      |
| 24            | 並列 AI 処理数                                                           | **確定（Issue #81）**: 設定値化（既定 4）。レート制限に合わせて調整                                                           |
...
| --- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | 手書き認識（綺麗／普通／汚い字）       | 採用 OCR（Issue #81 で Google Document AI に確定）の日本語手書き精度と Bounding Box の質。LLM 併用時の改善幅                                           |
