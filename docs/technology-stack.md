...
| 論点                                                        | 現時点の方針                                                                                                                                                                    | 出典                                                                                                          |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 答案データ（個人情報を含む）の外部 AI/OCR クラウド API 送信 | **可**。**Issue #95 決定 7 で「必要最小限に絞る」制約は外れた**（採点基準・添削資料・添削サンプル・生徒答案とも送ってよく、ページ全体の画像も可）。**ただし公開は引き続き禁止** | ユーザー確認済み／[Issue #95](https://github.com/HIKARU0627/Auto-Scoring/issues/95) 決定 7、業務ルール §2 (2) |
...
- クラウド AI/OCR 送信は許可されている。**Issue #95 決定 7 で「設問単位の必要最小限」という
  制約は外れた**: 採点基準・添削資料・添削サンプル・**生徒答案**とも送ってよく、
  **答案ページ全体の画像**も送ってよい（回答欄の解析と資料の構造抽出がこれを必要とする。
  簡易設計書 §4.3・§6.2・§26.1、業務ルール §2 (2)）。
- **緩和されたのは送信範囲だけで、「生徒識別情報は送らない」という方針は残す。**
  実資料では答案の氏名欄・生徒 ID 欄が空欄だったため現時点で送るものが無いというだけであり、
  **次の資料で埋まっている可能性がある**（業務ルール §2 (2)「除去を前提に設計しない」）。
