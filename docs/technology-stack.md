...
| 選択肢                                        | ライセンス                                                                                    | 影響                                                                 |
| --------------------------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **A. PyMuPDF を採用（推奨・要ライセンス費）** | AGPL or 商用                                                                                  | 実装が最短。塾向け非公開配布なら商用ライセンス費用が発生。要見積もり |
| B. オープンソース構成に置換                   | `pypdfium2`（Apache/BSD）でラスタライズ＋座標取得、`pypdf`（BSD）でオーバーレイ・スタンプ生成 | ライセンス費ゼロ。ただし注釈描画・座標 API を自前で組む工数が増える  |
