...
**告知は理由ごとに1行で、設問ごとではない。** 実際に会うのは「provider が1つ落ちて、
その答案の設問が全部同じ理由で落ちる」形である。設問ごとに出すと、説明している当の
図とその下のPDFを、12行のヘッダが押しのけることになる。行数は
`DagFailureGuidance` の数で上限が決まり、設問の名指しは5件まで（それ以上は
「ほか n 件」）。同じ理由の12件は同じことを言い、出口も同じなので、
**1行のほうが真でもある。**
