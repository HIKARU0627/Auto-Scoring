...
### 1.9 ジョブが新しいグラフversionを名乗り始めたらグラフを取り直す
...
`Job.dependency_graph_version` が、いま描いているグラフより**大きい**versionを
名乗り始めたら取り直す。「どれか1つでもグラフと食い違ったら」ではないのは、再起票が
古いジョブをCANCELLEDのまま残すためで、その条件だと以後ずっと真になり毎ティック
取り直してしまう。
